...
>
> 🔴 **L0-POC 实测证伪(2026-07-23,见 §12):prop.js 墙钟收益 ≈ 0(0.31→0.30s)。** 机制虽验证(varInitTypes 接通 → _js_add 18.4%→0、正确性保持),但内联路自身开销(coerce 守卫 ~15% + nan_canon 6.8% 叶采样)几乎 1:1 抵销;且杀 nan_canon 后墙钟仍不动——**叶采样 % 不转化为墙钟**(第四次实测确认,同 T2a/T2b/内联 POC)。prop.js 非算术瓶颈,是结构性差距(指令数/GP↔FP fmov 往返/I-cache)。下文 §1.1/§4.3/§10 的"~7.5–8.5×"收益预测**基于被证伪的"叶采样→墙钟"假设,仅作历史记录,不可当真**。
...
3. **⚠️ 原"低风险层(L0+L1+L2)~25% → prop.js 7.5–8.2×"预测已被 §12 POC 证伪(墙钟 ≈ 0)。** 叶采样占比(_js_add 18%、屏障 7%)**不转化为墙钟**;prop.js 实测非算术瓶颈,是结构性差距。本专项在 prop.js 类紧循环上无墙钟收益,价值(若有)须另寻算术密集/少读负载或代码体积维度验证。
...
- 估算(⚠️ **已被 §12 POC 证伪**):原预测 _js_add 18.4% 大部分消灭 → 10.3×→~8.5×;实测接通后 _js_add 归零但墙钟 ≈ 0(内联路 coerce 守卫 ~15% + nan_canon 6.8% 叶采样抵销,且杀 nan_canon 墙钟仍不动)。
...
- ~~**prop.js**:10.3× → L0 ~8.5×~~(**§12 证伪:墙钟 ≈ 0**);prop.js 非 L0 收益载体,改用自编译 _js_add 占比 + 算术密集(少读)微基准衡量;
- **prop.js 采样**:L0 接通后 _js_add 18.4%→0(机制生效),但墙钟不降——叶采样 ≠ 墙钟(§12);
...
## 12. L0-POC 实测证伪(2026-07-23,arch/efficiency)

**结论:L0(接通 varInitTypes)在 prop.js 上墙钟收益 ≈ 0。机制验证成功,价值命题(墙钟)证伪。POC 改动已回退,未提交。**

### 12.1 POC 做法

statements.js 声明处临时填充休眠钩子 `ctx.varInitTypes[name]={properties:{...}}`(对象字面量全数值初值,NUMBER),令 types.js:244 的 `inferType(o.b)` 返回 NUMBER → `o.b+o.c` 走 operators.js:1093 内联浮点路(绕 bl _js_add)。另测一变体:放宽 :1089 numTy 门(`===FLOAT64`→`isFloatType`,含 NUMBER)以杀 nan_canon。**两改均测量后回退**(numTy 是共享热路径 + nan-int0 别名风险,~0 收益不值得)。

### 12.2 实测数据

| 变体 | prop.js 墙钟 | _js_add 叶采样 | 新增开销 |
|---|---|---|---|
| 基线 | 0.31s(10.3×) | 18.4% | — |
| **POC1**(接 varInitTypes,NUMBER) | 0.29s(9.6×) | **0%(归零✓)** | coerce 守卫 `ncf_*` ~12% + nan_canon 6.8% |
| **POC2**(+放宽 numTy 杀 nan_canon) | 0.30s | 0% | nan_canon **0%**、coerce 守卫 ~15% |

- **正确性**:POC1/2 输出 60000000 == node,机制无正确性问题。
- **机制确认**:varInitTypes 接通 → _js_add 从 18.4% 彻底归零,内联路接管。**枢纽发现(休眠钩子)成立。**
- **价值证伪**:消灭 _js_add(18.4% 叶采样)仅换来 10.3×→9.6×(~6%);再杀 nan_canon(6.8% 叶采样)墙钟纹丝不动(0.29→0.30,噪声内)。

### 12.3 归因:叶采样 ≠ 墙钟(第四次实测确认)

内联路接管后,成本搬家而非消失:_js_add 的 bl/prologue/12 标签判别 → 换成 2×emitNumberCoerceFast 内联守卫(7 条/操作数,含堆范围检查,叶采样显为 `ncf_heap`/`ncf_done`)+ nan_canon call。而**即便把这些也消除,墙钟仍不动**——与既有三次证伪同源:

- T2a(IC miss 扫描消除):零效果(miss 是一次性事件);
- T2b(getter 检查短路):14.9% 叶采样 → 零墙钟差(调用边界聚集假象);
...
1. **prop.js 非 L0 收益载体,微基准收益井经第四次实测确认彻底枯竭。** 不应再为 prop.js 类紧循环投入 L0–L4。
2. L0 的**机制**(休眠 varInitTypes 钩子 + 内联路)真实存在且正确,但价值不在 prop.js 墙钟;**潜在价值维度**(未验证):① 算术密集/少读负载(读机制占比低时算术占比才显著);② 代码体积(_js_add call 站点 → 内联,可能减小产物——但与内联 POC 的 +144% 膨胀方向相反,需实测);③ 自编译热图 _js_add 占比下降是否转化为自编译墙钟(未测)。
3. **唯一未实测穷尽的结构性杠杆**:FP 值跨成员访问循环驻留(P4.1 路线 A,callee-saved d8-d15 + prologue/epilogue FP 保存),攻 GP↔FP fmov 往返——但 invasive、高自举风险、且 P4.1 下标读扩展已试过因 caller-saved 冲突回退(记忆 [[unboxing-int-residency-design]] 负结果)。
4. **建议:接受 AOT 层级微基准性能,把精力转回 plan.md 北极星(test262 覆盖率 20.55%→40%)**——这是确定有价值、非零和的方向。支柱① v2 设计(本文档)作为机制档案保留,待未来出现算术密集真实负载或精确 GC(支柱⑤,数值槽免扫)协同时再评估。

### 12.5 教训入库

- **休眠钩子 ≠ 性能杠杆**:varInitTypes 读侧就绪、写侧缺失是真实的架构缺口,补上它消除了 _js_add 分派——但分派本就不是墙钟瓶颈。**"接通休眠代码"的价值须先证其瓶颈地位,不能由"它休眠"推断。**
- **叶采样占比是定位线索,不是收益承诺**:本会话四次(T2a/T2b/内联/本次)均"高叶采样 → 零墙钟"。任何新杠杆立项前,先问"消除它墙钟会动吗",用小 POC 测,不押大投资。
- **prop.js 10.3× 是结构性的,非分派性的**:后续性能工作若仍以 prop.js/num.js 微基准为标靶,大概率重复证伪;标靶应换成真实负载(自编译墙钟、test262 吞吐)或明确的结构杠杆(FP 驻留/精确 GC)。

---

## 13. 路线 A 实测:FP callee-saved 驻留是真实杠杆(2026-07-23)

> 承 §12 战略修订第 3 点:FP 值跨调用循环驻留(P4.1 路线 A)是四次证伪后唯一未测尽的结构杠杆。**实测结论:杠杆为真——L4 之后首个正向微优化。** 与 §12 四次证伪**不同类**:T2a/T2b/内联/L0 消的是分支预测可廉价的 branch/call/标签判别;路线 A 消的是**真实数据搬运 uop**(累加器 slot 存取 + GP↔FP fmov),分支预测廉价化不了它。

### 13.1 机制与实现

callee-saved d8 驻留浮点累加器,覆盖 caller-saved d2-d6(P4.1 B)因 `_subtreeBlocksFpAccum` 拒绝的**含 call/成员循环**。关键事实:运行时 helper(`_object_get_ic`/`_number_coerce` 等)只用 d0-d7、**不碰 d8-d15** → d8 跨这些调用天然保活。routing 代码(assignments.js:99-127 `fadd(fpReg,...)`、members.js:457)对寄存器号参数化,pin d8(reg=8)即复用;vm FP 指令(fadd/fmovToFloat)把 fpReg 当 5 位 raw d-reg 号,d8=8 直接编码。

检测:`detectFpAccumVarsCallee`(statements.js)——detectFpAccumVars(caller-saved)因含 call/成员 bail 时的 fallback,pin 单累加器到 d8,保留 STRING/boxed/rawInt/local 守卫 + 拒 test/update 引用。物化:循环入口 `load slot→fmovToFloat(8)`、出口 `fmovToInt(8)→store slot`(reg 参数化,复用 P4.1)。POC 经 `process.env.FP_CALLEE_POC` 编译期门隔离 fixtures。

### 13.2 实测收益(accumulate 主导 bench,50M 迭代)

| bench | 形态 | base(slot) | 路线 A(d8) | 收益 | node |
|---|---|---|---|---|---|
| red-member | `s=s+o.x`(1 IC 读/迭代) | 0.20s(5.0×) | 0.16-0.17s(4.2×) | **~15-20%(带噪声)** | 0.04s |
| red-call | `s=s+inc(1.0)`(1 用户调用/迭代) | 0.25-0.26s(6.2×) | 0.18s(4.5×) | **~28-31%(稳定)** | 0.04s |

**收益随 in-loop 调用开销增大**(用户调用 > IC 读):调用越重,累加器 slot 往返被调用掩盖得越多,驻留省的相对越多。

### 13.3 soundness:d8-vs-d8 互调 ✓,异常展开 ✗(阻塞)

- **d8-vs-d8 互调 sound**:内外层函数各持含成员访问的 d8 累加器,外层累加活跃时调内层——循环级 `push(入口存调用者 d8)/pop(出口恢复)`(仅现有指令,无需 asm FP 编码/prologue 机制)正确保活:内层入口存的正是外层累加器值。实测 red-sound(1000×1.5 + inner(100)×2.0)= **1700 三方一致** ✓。
- **🔴 异常展开阻塞(实证)**:循环体调用抛异常函数、try 在循环外 → 异常展开**跳过循环出口的 d8→slot 回写** → 累加器值丢失。实测 red-throw:node/base = 126.5,路线 A = **0**(错)。非栈失衡,是**值丢**——累加器全程驻留 d8,异常绕过出口物化,slot 停在初值。

### 13.4 交付路径(分裂)

| 方案 | 覆盖 | 收益 | 风险 |
|---|---|---|---|
| **(a) 保守门**:拒 body 含 CallExpression(用户调用可抛)、允 MemberExpression(IC 读 `_object_get_ic` 不抛) | 仅 IC 读累加循环 | red-member 类 ~15%(噪声内) | 低,可即过 fixtures |
| **(b) 异常帧集成**:累加器在异常展开时回写(对接 #38 异常帧契约) | 全部含调用累加循环 | red-call 类 ~30%(完整) | **高**(第 4 层,异常机制深耦合) |

方案 (a) 可即交付但收益边际(且 red-call 的 ~30% 被排除);方案 (b) 给完整收益但高风险。**当前实现 = 测量 POC(env 门、循环级 push/pop、非异常 sound),未提交默认开启。**

### 13.5 战略意义

1. **FP 驻留经实测确认为真实杠杆**——终结"微基准收益井是否彻底枯竭"之问:**枯竭的是分支预测类(dispatch/check/read 内联),数据搬运类(FP 驻留)仍有 ~15-30%。** 二者机制不同,不可由前者证伪推后者。
2. 完整收益的瓶颈是**异常处理**(累加器跨异常回写),非算术/查找——又一次指向运行时机制深耦合是性能工作的真正成本。
3. 决策岔口:(a) 投异常帧集成换取 ~30% 真实提速(高性能但高风险、配额受限);(b) 交付保守 IC 读子集(~15%,边际);(c) 存档本发现,转 plan.md 北极星 test262(确定高价值、非零和)。本设计稿+实测数据为 (a)/(b) 的完整施工图,待决策后执行。
