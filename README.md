...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.13** — `Array.prototype`'s missing methods (`find`/`findIndex`/`fill`/`flat`/`flatMap`/`concat`/`copyWithin`/`keys`/`values`/`entries`) are now first-class method values (previously read as `undefined`), working in method-value/`.call`/`Array.prototype.<m>.call` forms with correct own `name`/`length`, and `keys`/`values`/`entries` return real iterators — lifting the overall stride-5 subset to **47.09%** (3,043/6,462, +7) with built-ins/Array 48.8% → 50.0% and CRASH −3; the fixture suite stays green (385/0) and the build holds its `gen1==gen2==gen3` fixed point. Recent prior releases: **v0.3.12** (builtin functions first-class — own `name`/`length` + `[[Set]]`/`[[Delete]]` semantics, +70); **v0.3.11** (parser early-error enforcement); **v0.3.10** (Date materialized); **v0.3.9/v0.3.8/v0.3.7/v0.3.6/v0.3.5** (gOPD array support; wrapper enumeration; defineProperty semantics + enforcement; engineering-truth/security wave) — all building on the prior conformance + security hardening (class generator/async-generator coroutines, `typeof <undeclared>` → `"undefined"`, `propertyHelper.js` harness loading, 19 parser precedence sites, global `Buffer`, uncaught-exception stderr diagnostics, and closed out-of-bounds TypedArray/DataView access, GCM auth-tag bypass, zlib inflate hang and `ar` command injection), all on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (385/385 — green for the first time); a test262 conformance harness is in place — current baseline 3,043 / 6,462 = 47.09% on a stride-5 subset (`language/` + 13 core `built-ins/`; built-ins/Object 57.6%, Math 69.2%, Array 50.0%, String 38.5%; see `tests/test262/last_report.md`; reproducible via a pinned corpus + `runnerSha`/`corpusPin`), up from 20.55% at v0.2.1, with CRASH at ~159 and COMPILE_FAIL at ~69 — still a low number that is being actively raised.
