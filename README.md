...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.15** — the bare `JSON` identifier materializes as a real namespace object with spec-attribute `parse`/`stringify` method values, `String.fromCharCode`/`fromCodePoint` become first-class method values, and the bare `Number` identifier gains a constructor object with six statics (`isInteger`/`isFinite`/`isNaN`/`isSafeInteger`/`parseFloat`/`parseInt`) and eight attr-0 constants; plus a crash-elimination wave: `String(d)`/template-embedding/`"foo" in d` on dates, zero-arg Date setters, `d.setSeconds('abc')` silent corruption, Invalid-Date getters, and closures capturing `Object.prototype`-named locals (`valueOf`, `toString`, …) no longer SIGSEGV/SIGBUS or misread — lifting the stride-5 subset to **47.77%** (3,087/6,462, +24) with built-ins/Number 36.8% → 44.1% and CRASH 163 → 158, while the non-scored built-ins/Date directory reaches 67.9% (396/583, stride-1); the fixture suite stays green (385/0) and the build holds its `gen1==gen2==gen3` fixed point. Recent prior releases: **v0.3.14** (receiver-guard hardening for Map/Set/Promise/Date, parser object-pattern early errors, lexer-aware CJS detection, +20); **v0.3.13** (`Array.prototype`'s missing methods first-class, +7); **v0.3.12** (builtin functions first-class — own `name`/`length` + `[[Set]]`/`[[Delete]]` semantics, +70); **v0.3.11** (parser early-error enforcement); **v0.3.10/v0.3.9/v0.3.8/v0.3.7/v0.3.6/v0.3.5** (Date materialized; gOPD array support; wrapper enumeration; defineProperty semantics + enforcement; engineering-truth/security wave) — all building on the prior conformance + security hardening (class generator/async-generator coroutines, `typeof <undeclared>` → `"undefined"`, `propertyHelper.js` harness loading, 19 parser precedence sites, global `Buffer`, uncaught-exception stderr diagnostics, and closed out-of-bounds TypedArray/DataView access, GCM auth-tag bypass, zlib inflate hang and `ar` command injection), all on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (385/385 — green for the first time); a test262 conformance harness is in place — current baseline 3,087 / 6,462 = 47.77% on a stride-5 subset (`language/` + 13 core `built-ins/`; built-ins/Object 58.5%, Math 69.2%, Map 51.2%, Set 52.6%, Array 50.3%, Number 44.1%; see `tests/test262/last_report.md`; reproducible via a pinned corpus + `runnerSha`/`corpusPin`), up from 20.55% at v0.2.1, with CRASH at ~158 and COMPILE_FAIL at ~69 — still a low number that is being actively raised.
...
Every non-ARM64 fix is gated by target so the ARM64 outputs stay byte-identical. The
fixed point is re-verified to `gen2 == gen3` on both ARM64 targets after each change;
the three x64 targets reached it at v1.1.0 but currently regress on full self-compilation
(devirtualization is disabled for x64 pending the layout-blocker resolution).
