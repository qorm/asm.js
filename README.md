...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.12** — builtin functions made first-class: builtin method values (`Array.prototype.*`, `String.prototype.*`, `Object.prototype.*`, and instance method refs) now carry correct own `name`/`length` with descriptors `{writable:false, enumerable:false, configurable:true}`, and function `name`/`length` now enforce `[[Set]]` (writes ignored, matching Node's non-writable semantics) and `[[Delete]]` (removed permanently via a tombstone rather than revived by the metadata fallback) — lifting the overall stride-5 subset to **46.98%** (3,036/6,462, +70 over the v0.3.10 baseline) with built-ins/Object 57.6%, Math 69.2%, Array 48.8%, String 38.5%, and zero over-rejection; the fixture suite stays green (385/0) and the build holds its `gen1==gen2==gen3` fixed point. Recent prior releases: **v0.3.11** (parser early-error enforcement); **v0.3.10** (Date materialized as a real runtime object); **v0.3.9/v0.3.8/v0.3.7/v0.3.6** (gOPD array support; wrapper enumeration; defineProperty semantics + field-presence-mask enforcement); **v0.3.5** (engineering-truth/security wave) — all building on the prior conformance + security hardening (class generator/async-generator coroutines, `typeof <undeclared>` → `"undefined"`, `propertyHelper.js` harness loading, 19 parser precedence sites, global `Buffer`, uncaught-exception stderr diagnostics, and closed out-of-bounds TypedArray/DataView access, GCM auth-tag bypass, zlib inflate hang and `ar` command injection), all on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (385/385 — green for the first time); a test262 conformance harness is in place — current baseline 3,036 / 6,462 = 46.98% on a stride-5 subset (`language/` + 13 core `built-ins/`; built-ins/Object 57.6%, Math 69.2%, Array 48.8%, String 38.5%; see `tests/test262/last_report.md`; reproducible via a pinned corpus + `runnerSha`/`corpusPin`), up from 20.55% at v0.2.1, with CRASH at ~159 and COMPILE_FAIL at ~69 — still a low number that is being actively raised.
