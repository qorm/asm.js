...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.10** — the `Date` builtin is now a real runtime object: `typeof Date === "function"`, `Date.prototype` is a real object, and `Object.getOwnPropertyDescriptor` reflects `Date`'s statics (`now`/`parse`/`UTC`) and all `Date.prototype` methods with correct attributes (statics `writable:true,enumerable:false,configurable:true`, `prototype` all-false), with generic `.call` working; built-ins/Object to **56.9%** (388/682) and the overall stride-5 subset to **44.91%** (2,902/6,462); the fixture suite stays green (385/0) and the build holds its `gen1==gen2==gen3` fixed point. All existing Date behavior is preserved (`new Date()`, `Date.now()`, getters/setters, `instanceof Date`). Known residuals (built-ins/Date, not in the scored subset): 11 edge-case SIGSEGVs now reachable via the prototype — non-object `this` (missing this-type checks), setter argument coercion, `Symbol.toPrimitive`, extended-year `toString`, and `class extends Date` — documented for a Date-hardening follow-up. Recent prior releases: **v0.3.9** (gOPD array support + undefined/null TypeError); **v0.3.8** (wrapper enumeration fixed); **v0.3.7/v0.3.6** (defineProperty semantics + field-presence-mask enforcement); **v0.3.5** (engineering-truth/security wave) — all building on the prior conformance + security hardening (class generator/async-generator coroutines, `typeof <undeclared>` → `"undefined"`, `propertyHelper.js` harness loading, 19 parser precedence sites, global `Buffer`, uncaught-exception stderr diagnostics, and closed out-of-bounds TypedArray/DataView access, GCM auth-tag bypass, zlib inflate hang and `ar` command injection), all on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (385/385 — green for the first time); a test262 conformance harness is in place — current baseline 2,902 / 6,462 = 44.91% on a stride-5 subset (`language/` + 13 core `built-ins/`, built-ins/Object 56.9%; see `tests/test262/last_report.md`; reproducible via a pinned corpus + `runnerSha`/`corpusPin`), up from 20.55% at v0.2.1, with CRASH at ~159 and COMPILE_FAIL at ~69 — still a low number that is being actively raised.
