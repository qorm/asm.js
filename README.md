...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (385/385 — green for the first time); a test262 conformance harness is in place — current baseline 3,782 / 6,462 = 58.68% on a stride-5 subset (`language/` + 13 core `built-ins/`; built-ins/Math 69.2%, Number 60.3%, Object 62.2%, JSON 60.6%, RegExp 46.3%, Promise 48.0%, Map 51.2%, Set 48.7%, Array 55.9%; see `tests/test262/last_report.md`; reproducible via a pinned corpus + `runnerSha`/`corpusPin`), up from 20.55% at v0.2.1, with CRASH at ~155 and COMPILE_FAIL at ~57 — still a low number that is being actively raised.
