...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.7** — completes the Object.defineProperty descriptor semantics begun in v0.3.6: non-object descriptors and mixed data+accessor descriptors are now rejected on the dynamic path too, and changing the value of a non-configurable, non-writable property now throws (with a content-based string `SameValue`, so `"ab"` and `"a"+"b"` compare equal), lifting built-ins/Object from 53.7% to **55.0%** and the overall stride-5 subset to **44.69%** (2,888/6,462); the fixture suite stays green (385/0) and the build holds its `gen1==gen2==gen3` fixed point. The prior release, **v0.3.6**, introduced that enforcement via a field-presence-mask `ValidateAndApplyPropertyDescriptor` (only the fields a descriptor actually specifies are enforced) plus compile-time `ToPropertyDescriptor` validation (non-callable `get`/`set` and mixed data+accessor descriptors throw `TypeError`). **v0.3.5** was the engineering-truth, security and correctness wave (truthful fixture gate with the suite green for the first time at 0 XFAIL, single-sourced target catalog and current facts, fail-closed crypto/vm/linking/temp-files, two negative-fixture product gaps fixed, reproducible pinned-corpus test262 at 43.89%) — all building on the prior conformance + security hardening (class generator/async-generator coroutines, `typeof <undeclared>` → `"undefined"`, `propertyHelper.js` harness loading, 19 parser precedence sites, global `Buffer`, uncaught-exception stderr diagnostics, and closed out-of-bounds TypedArray/DataView access, GCM auth-tag bypass, zlib inflate hang and `ar` command injection), all on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (385/385 — green for the first time); a test262 conformance harness is in place — current baseline 2,888 / 6,462 = 44.69% on a stride-5 subset (`language/` + 13 core `built-ins/`, built-ins/Object 55.0%; see `tests/test262/last_report.md`; reproducible via a pinned corpus + `runnerSha`/`corpusPin`), up from 20.55% at v0.2.1, with CRASH at ~159 and COMPILE_FAIL at ~69 — still a low number that is being actively raised.
