...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.17** — the x64 backend register-aliasing (`V0≡RET≡LR`) correctness wave: 52 sites where arm64-era code used `V0` as a free scratch (clobbering `RET` on x64) are re-homed with per-site liveness proofs, restoring the entire first-class reflection surface (`typeof`/`X===X`/prototypes/method values across 11 builtin families) on all three x64 targets; plus red-team-surfaced siblings (`V1≡A3` in DataView/TypedArray `get`/`set`/`fill`/`copyWithin`), two x64 emission-layer fixes (`Math.max(−0,0)`, `asin`/`acos`/`atan`), and activation of Date instance method reads on x64 — test262 holds **47.77%** (3,087/6,462, byte-identical to v0.3.16, as expected for a register-domain wave), with `vm/registers.js` aliasing defenses and a reusable scanner landed. Recent prior releases: **v0.3.16** (Date instance method values, `String()` brand conversions, Date direct-path finishing, `TA_BPE` pollution guard); **v0.3.15** (JSON/Number/String first-class materialization + crash elimination, +24); **v0.3.14** (receiver-guard hardening, parser object-pattern early errors, lexer-aware CJS detection, +20); **v0.3.13/v0.3.12/v0.3.11/v0.3.10/v0.3.9/v0.3.8/v0.3.7/v0.3.6/v0.3.5** (`Array.prototype` methods first-class; builtin functions first-class; parser early-error enforcement; Date materialized; gOPD array support; wrapper enumeration; defineProperty semantics + enforcement; engineering-truth/security wave) — all building on the prior conformance + security hardening (class generator/async-generator coroutines, `typeof <undeclared>` → `"undefined"`, `propertyHelper.js` harness loading, 19 parser precedence sites, global `Buffer`, uncaught-exception stderr diagnostics, and closed out-of-bounds TypedArray/DataView access, GCM auth-tag bypass, zlib inflate hang and `ar` command injection), all on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
