...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.18** — global function values first-class (`parseInt`/`isNaN`/`typeof` now `"function"`, `Number.parseInt === parseInt`), `JSON.rawJSON`/`isRawJSON` and `String.raw` (with the JSON error-brand family aligned to `SyntaxError`), and `Number.prototype` materialized — six methods work as same-slot method values (`(5).toFixed === Number.prototype.toFixed`), guarded against wrong receivers, with `gOPN(Number)`/`gOPN(Number.prototype)` matching Node exactly and `(NaN).toFixed(2)` returning `"NaN"` — lifting the stride-5 subset to **48.16%** (3,112/6,462, +25) with built-ins/Number 44.1% → 60.3% (CRASH 1 → 0) and built-ins/JSON 30.3% → 57.6%, zero area regressions; fixtures green (385/0), `gen1==gen2==gen3` fixed point holds. Recent prior releases: **v0.3.17** (x64 register-aliasing `V0≡RET` correctness wave — first-class reflection surface restored on x64 targets); **v0.3.16** (Date instance method values, `String()` brand conversions, Date direct-path finishing, `TA_BPE` pollution guard); **v0.3.15** (JSON/Number/String first-class materialization + crash elimination, +24); **v0.3.14/v0.3.13/v0.3.12/v0.3.11/v0.3.10/v0.3.9/v0.3.8/v0.3.7/v0.3.6/v0.3.5** (receiver-guard hardening; parser early-error enforcement; `Array.prototype` methods first-class; builtin functions first-class; Date materialized; gOPD array support; wrapper enumeration; defineProperty semantics + enforcement; engineering-truth/security wave) — all building on the prior conformance + security hardening (class generator/async-generator coroutines, `typeof <undeclared>` → `"undefined"`, `propertyHelper.js` harness loading, 19 parser precedence sites, global `Buffer`, uncaught-exception stderr diagnostics, and closed out-of-bounds TypedArray/DataView access, GCM auth-tag bypass, zlib inflate hang and `ar` command injection), all on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (385/385 — green for the first time); a test262 conformance harness is in place — current baseline 3,112 / 6,462 = 48.16% on a stride-5 subset (`language/` + 13 core `built-ins/`; built-ins/Math 69.2%, Number 60.3%, Object 58.8%, JSON 57.6%, Set 52.6%, Map 51.2%, Array 50.3%; see `tests/test262/last_report.md`; reproducible via a pinned corpus + `runnerSha`/`corpusPin`), up from 20.55% at v0.2.1, with CRASH at ~161 and COMPILE_FAIL at ~69 — still a low number that is being actively raised.
