...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (398/398 — green); a test262 conformance harness is in place — current workspace baseline 4,431 / 6,313 = **70.19%** on a stride-5 subset (`language/` + 13 core `built-ins/`; built-ins/Math 81.5%, Number 82.1%, Object 80.0%, JSON 64.5%, String 73.3%, RegExp 61.2%, Promise 61.4%, Map 78.0%, Set 67.1%, Array 67.2%; see `tests/test262/last_report.md`; reproducible via a pinned corpus + `runnerSha`/`corpusPin`), up from 20.55% at v0.2.1, with CRASH at 70 and COMPILE_FAIL at 43 — still being actively raised.
