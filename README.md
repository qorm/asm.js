...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.0** — a test262 conformance push (**20.55% → 39.74%** on the stride-5 subset, crashes 480 → 181) plus a security-hardening pass: class generator/async-generator methods now build real coroutines instead of segfaulting, `typeof <undeclared>` correctly yields `"undefined"`, test262's `propertyHelper.js` harness loads for the first time (generic function-invoke helper making `Function.prototype.call/apply` first-class), 19 parser precedence sites fixed, global `Buffer` materialized, uncaught exceptions now print a diagnostic to stderr instead of exiting silently, and out-of-bounds TypedArray/DataView access, a GCM auth-tag bypass, a zlib inflate hang and an `ar` command injection all closed — on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (380/380); a test262 conformance harness is in place — current baseline 2,568 / 6,462 = 39.74% on a stride-5 subset (`language/` + 13 core `built-ins/`, see `tests/test262/last_report.md`), up from 20.55% at v0.2.1, with CRASH down to 181 and COMPILE_FAIL to 40 — still a low number that is being actively raised.
