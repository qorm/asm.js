...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.6** — Object.defineProperty descriptor semantics: compile-time `ToPropertyDescriptor` validation (non-callable `get`/`set`, mixed data+accessor descriptors throw `TypeError`) plus runtime `ValidateAndApplyPropertyDescriptor` enforcement driven by a field-presence mask (redefining non-configurable properties, adding keys to non-extensible objects, and accessor/data switches now throw as the spec requires), lifting built-ins/Object from 46.3% to **53.7%** and the overall stride-5 subset to **44.55%** (2,879/6,462, up from 43.89%); the fixture suite stays green (385/0) and the build holds its `gen1==gen2==gen3` fixed point. The prior release, **v0.3.5**, was the engineering-truth, security and correctness wave: the fixture gate was made truthful (single authoritative runner; the 385-fixture suite green for the first time, 0 XFAIL), the target catalog and current facts single-sourced, security closed fail-closed (crypto `createHash`/`createHmac` reject unknown algorithms with the fake-digest path removed, `createECDH`/`setFips(true)` throw, `randomInt` rejection-sampling, `vm` no longer swallows errors, `--lib`/`--lib-path` fail loudly, 0700 mkdtemp temp files), two long-standing negative-fixture product gaps fixed (unknown `node:` builtins and ESM-in-`type:"commonjs"` now rejected at compile time), and the test262 harness made reproducible (corpus pinned to a commit with `runnerSha`/`corpusPin`); test262 at 43.89% (2,836/6,462 stride-5) — all building on the prior conformance + security hardening (class generator/async-generator coroutines, `typeof <undeclared>` → `"undefined"`, `propertyHelper.js` harness loading, 19 parser precedence sites, global `Buffer`, uncaught-exception stderr diagnostics, and closed out-of-bounds TypedArray/DataView access, GCM auth-tag bypass, zlib inflate hang and `ar` command injection), all on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (385/385 — green for the first time); a test262 conformance harness is in place — current baseline 2,879 / 6,462 = 44.55% on a stride-5 subset (`language/` + 13 core `built-ins/`, built-ins/Object 53.7%; see `tests/test262/last_report.md`; reproducible via a pinned corpus + `runnerSha`/`corpusPin`), up from 20.55% at v0.2.1, with CRASH at ~159 and COMPILE_FAIL at ~69 — still a low number that is being actively raised.
