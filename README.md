...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (380/380); a test262 conformance harness is in place — current baseline 2,390 / 6,462 = 36.99% on a stride-5 subset (`language/` + 13 core `built-ins/`, see `tests/test262/last_report.md`), up from 20.55% at v0.2.1, with CRASH down to 175 and COMPILE_FAIL to 40 — still a low number that is being actively raised.
