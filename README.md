...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.8** — exotic wrapper objects no longer leak internal slots into enumeration: `Object.keys`/`values`/`entries`/`for-in`/`JSON.stringify` over a `RegExp` or `new String(...)` now match Node (internal slots are own but non-enumerable), and `Object.assign`/object spread now honor enumerability — reclaiming the three test262 cases that v0.3.7's stricter descriptor validation had exposed, lifting built-ins/Object to **55.4%** (378/682) and the overall stride-5 subset to **44.75%** (2,892/6,462); the fixture suite stays green (385/0) and the build holds its `gen1==gen2==gen3` fixed point. Recent prior releases: **v0.3.7** completed the Object.defineProperty descriptor semantics (dynamic non-object/mixed descriptors rejected; value-change on non-configurable non-writable props throws with content-based string SameValue); **v0.3.6** introduced that enforcement via a field-presence-mask `ValidateAndApplyPropertyDescriptor` plus compile-time `ToPropertyDescriptor` validation; **v0.3.5** was the engineering-truth, security and correctness wave (truthful fixture gate green for the first time at 0 XFAIL, single-sourced target catalog/facts, fail-closed crypto/vm/linking/temp-files, two negative-fixture gaps fixed, reproducible pinned-corpus test262 at 43.89%) — all building on the prior conformance + security hardening (class generator/async-generator coroutines, `typeof <undeclared>` → `"undefined"`, `propertyHelper.js` harness loading, 19 parser precedence sites, global `Buffer`, uncaught-exception stderr diagnostics, and closed out-of-bounds TypedArray/DataView access, GCM auth-tag bypass, zlib inflate hang and `ar` command injection), all on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (385/385 — green for the first time); a test262 conformance harness is in place — current baseline 2,892 / 6,462 = 44.75% on a stride-5 subset (`language/` + 13 core `built-ins/`, built-ins/Object 55.4%; see `tests/test262/last_report.md`; reproducible via a pinned corpus + `runnerSha`/`corpusPin`), up from 20.55% at v0.2.1, with CRASH at ~159 and COMPILE_FAIL at ~69 — still a low number that is being actively raised.
