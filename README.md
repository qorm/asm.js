...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.9** — `Object.getOwnPropertyDescriptor` now handles arrays, synthesizing the spec `length` descriptor (`{writable:true, enumerable:false, configurable:false}`) and indexed-element descriptors (`{writable:true, enumerable:true, configurable:true}`), including for `arguments`, and throws `TypeError` on `undefined`/`null` targets as Node does; built-ins/Object to **55.6%** (379/682) and the overall stride-5 subset to **44.77%** (2,893/6,462); the fixture suite stays green (385/0) and the build holds its `gen1==gen2==gen3` fixed point. Recent prior releases: **v0.3.8** (exotic wrapper enumeration fixed — RegExp/new String internal slots non-enumerable, `Object.assign` honors enumerability); **v0.3.7** (Object.defineProperty descriptor semantics completed); **v0.3.6** (field-presence-mask enforcement + `ToPropertyDescriptor` validation); **v0.3.5** (engineering-truth/security wave — truthful fixture gate first all-green at 0 XFAIL, single-sourced catalog/facts, fail-closed crypto/vm/linking/temp-files, reproducible pinned-corpus test262) — all building on the prior conformance + security hardening (class generator/async-generator coroutines, `typeof <undeclared>` → `"undefined"`, `propertyHelper.js` harness loading, 19 parser precedence sites, global `Buffer`, uncaught-exception stderr diagnostics, and closed out-of-bounds TypedArray/DataView access, GCM auth-tag bypass, zlib inflate hang and `ar` command injection), all on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
...
- **Pre-production.** No stability guarantees, no semver discipline yet, one primary developer. Test coverage is primarily fixture-based (385/385 — green for the first time); a test262 conformance harness is in place — current baseline 2,893 / 6,462 = 44.77% on a stride-5 subset (`language/` + 13 core `built-ins/`, built-ins/Object 55.6%; see `tests/test262/last_report.md`; reproducible via a pinned corpus + `runnerSha`/`corpusPin`), up from 20.55% at v0.2.1, with CRASH at ~159 and COMPILE_FAIL at ~69 — still a low number that is being actively raised.
