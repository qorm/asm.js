...
A self-hosting, zero-dependency JavaScript→native AOT compiler (5 targets: macOS/Linux arm64+x64, Windows x64). Latest release **v0.3.16** — Date instance method values (`var f = d.getFullYear` now works for the full 35-method family with `d.m === Date.prototype.m` identity), `String()`/template conversions print proper `[object Map]`/`[object Set]`/`[object Promise]`/… brands instead of garbage floats, Date direct-path finishing (direct `setTime` TimeClip, float64-domain MakeTime with bit-exact V8 FMA-contraction emulation for huge/cancelling arguments, Invalid-receiver setters returning NaN), and a `TA_BPE` prototype-pollution guard; test262 holds **47.77%** (3,087/6,462, PASS byte-parity with v0.3.15; a CRASH +4 wobble is attributed to host-load nondeterminism — same-tree re-runs diverge by ±17 PASS — with the fixed point, fixtures, and all red-team byte batteries deterministically green). Recent prior releases: **v0.3.15** (JSON/Number/String first-class materialization + Date/String-conversion and closure prototype-pollution crash elimination, +24); **v0.3.14** (receiver-guard hardening for Map/Set/Promise/Date, parser object-pattern early errors, lexer-aware CJS detection, +20); **v0.3.13** (`Array.prototype`'s missing methods first-class, +7); **v0.3.12** (builtin functions first-class — own `name`/`length` + `[[Set]]`/`[[Delete]]` semantics, +70); **v0.3.11/v0.3.10/v0.3.9/v0.3.8/v0.3.7/v0.3.6/v0.3.5** (parser early-error enforcement; Date materialized; gOPD array support; wrapper enumeration; defineProperty semantics + enforcement; engineering-truth/security wave) — all building on the prior conformance + security hardening (class generator/async-generator coroutines, `typeof <undeclared>` → `"undefined"`, `propertyHelper.js` harness loading, 19 parser precedence sites, global `Buffer`, uncaught-exception stderr diagnostics, and closed out-of-bounds TypedArray/DataView access, GCM auth-tag bypass, zlib inflate hang and `ar` command injection), all on top of shape (hidden-class) inline-cache infrastructure, TypedArray constructor globals, compile-time devirtualization of statically-resolvable method calls (self-compile −7.5%), G-M-P N>2 generalized work-stealing + an N-way stop-the-world GC across 3 real threads (linux-arm64), NUL-transparent strings, AES-GCM crypto, the test262 harness, real zlib / TCP, full compiler determinism (`gen1==gen2==gen3`), and full async. See **[CHANGELOG.md](./CHANGELOG.md)** for the full version history.
