...
自举、零依赖的 JavaScript→原生 AOT 编译器(5 目标:macOS/Linux arm64+x64、Windows x64)。最新版 **v0.3.8** — 奇异包装对象不再把内部槽泄入枚举:对 `RegExp` 或 `new String(...)` 的 `Object.keys`/`values`/`entries`/`for-in`/`JSON.stringify` 现与 Node 一致(内部槽为自有但不可枚举),`Object.assign`/对象展开现遵循可枚举性——收回 v0.3.7 收紧描述符校验时暴露的 3 个 test262 用例,built-ins/Object 升至 **55.4%**(378/682)、整体 stride-5 子集升至 **44.75%**(2,892/6,462);fixture 套件保持全绿(385/0)、`gen1==gen2==gen3` 定点保持。近几版:**v0.3.7** 补全 Object.defineProperty 描述符语义(动态非对象/混用描述符拒绝、non-configurable + non-writable 改值按内容字符串 SameValue 抛错);**v0.3.6** 引入该强制(字段存在位掩码 `ValidateAndApplyPropertyDescriptor` + 编译期 `ToPropertyDescriptor` 校验);**v0.3.5** 是工程真值、安全与正确性波次(fixture gate 真值化、套件首次全绿 0 XFAIL、目标目录/事实单源化、crypto/vm/链接/临时文件失败关闭、两个负向 fixture 缺口修复、test262 经固定 corpus 可复现至 43.89%)——均建立在既有符合性 + 安全加固(类 生成器/异步生成器协程、`typeof <未声明>` → `"undefined"`、`propertyHelper.js` harness 加载、19 处解析器优先级、全局 `Buffer`、未捕获异常 stderr 诊断,并闭合 TypedArray/DataView 越界访问、GCM 认证绕过、zlib inflate 死循环与 `ar` 命令注入)之上,全部建立在形状(隐藏类)内联缓存基础设施、 TypedArray 构造器全局值、静态可解析方法调用的编译期去虚拟化(自编译 −7.5%)、G-M-P N>2 通用工作窃取 + 跨 3 个真线程的 N 路停止世界 GC(linux-arm64)、NUL 透明字符串、AES-GCM 加密、test262 harness、真 zlib / TCP、完整编译器确定性(`gen1==gen2==gen3`)、完整 async 之上。完整版本历史见 **[CHANGELOG.zh-CN.md](./CHANGELOG.zh-CN.md)**。
...
- **未达生产级。** 尚无稳定性承诺与 semver 纪律,主力开发者一人。测试覆盖以 fixtures 为主(385/385——首次全绿);test262 符合性 harness 已就位,当前基线为 stride-5 子集(`language/` + 13 个核心 `built-ins/`,built-ins/Object 55.4%)2,892 / 6,462 = 44.75%(见 `tests/test262/last_report.md`;经固定 corpus + `runnerSha`/`corpusPin` 可复现),较 v0.2.1 的 20.55% 已显著提升,CRASH ~159、COMPILE_FAIL ~69;数字仍低、正在持续提升。
