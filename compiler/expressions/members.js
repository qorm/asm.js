...
// Runtime/compiler sources deliberately use the UTF-8 backing length for
// parser/code-generator bookkeeping. User modules, however, observe the
// ECMAScript String length (UTF-16 code units). Keep the distinction at the
// lowering boundary instead of changing the hot internal `_str_length` ABI.
function isInternalStringSourcePath(filePath) {
    if (typeof filePath !== "string") return false;
    // Module AST filenames are absolute in the host compiler but remain
    // relative (e.g. `runtime/node/__json_shim.js`) in gen1 self-hosting.
    // Treat both spellings as internal; otherwise the UTF-16 public length
    // path leaks into byte-oriented parser/JSON shims.
    return filePath.indexOf("/runtime/") !== -1 ||
        filePath.indexOf("runtime/") === 0 ||
        filePath.indexOf("/compiler/") !== -1 ||
...
// A RegExp exec result is a real Array whose numeric entries are strings.
// Keep that fact local to the member-index lowering instead of widening the
// global type lattice: `var m = re.exec(s); m[0][0]` is common in the unicode
// tests, and the second index must use UTF-16 units even though the engine's
// generic string subscript helper is byte-oriented for self-hosted sources.
function isRegExpExecCallNode(node, ctx) {
    if (!node || node.type !== "CallExpression" || !node.callee ||
        node.callee.type !== "MemberExpression" || node.callee.computed) return false;
    const prop = node.callee.property;
    if (!prop || prop.name !== "exec") return false;
    const recv = node.callee.object;
    if (!recv) return false;
    if (recv.type === "RegexLiteral") return true;
    if (recv.type === "Identifier" && ctx && ctx.getVarType) {
        return ctx.getVarType(recv.name) === "RegExp";
    }
    return false;
}

function isRegExpResultElement(node, ctx) {
    if (!node || node.type !== "MemberExpression" || !node.computed) return false;
    const prop = node.property;
    if (!prop || (prop.type !== "Literal" && prop.type !== "NumericLiteral") ||
        typeof prop.value !== "number" || Math.trunc(prop.value) !== prop.value) return false;
    if (isRegExpExecCallNode(node.object, ctx)) return true;
    const base = node.object;
    if (base && base.type === "Identifier" && ctx && ctx.varInitExprs &&
        isRegExpExecCallNode(ctx.varInitExprs[base.name], ctx)) return true;
    return false;
}

// [!! 本文件注释铁律 !!] 历史上 import.meta.url 分支用过 `.replace(/\\/g,"/")`,
// 自举依赖 compiler/index.js 把 RegExp shim 注入本文件。注入否决条件:文件名含
// shim 文件名、或源码里出现 shim 的**模块名字面量**——本文件注释/字符串绝不能写
// 出那个模块名(写了就跳过注入 → __RE_replace 无定义 → gen1 SIGSEGV)。同理勿写
// RegExp 构造探测串。当前该分支已改为手写扫反斜杠(无正则字面量),铁律仍保留
// 防回归:日后若再引入正则字面量必须先确认 shim 注入。
...
const BUILTIN_CTOR_META = {
    Object: 1, Function: 1, Array: 1, String: 1, Number: 1, Boolean: 1, Symbol: 0, BigInt: 1,
...
// [m119] vm/registers.js 的 VReg 键值同形(VReg.RET==="RET")。成员读折成串字面量,
// 自编译数万站免 load-capture + IC。查表用 ===1,避 Object.prototype。
const HOT_VREG_KEYS = {
    V0: 1, V1: 1, V2: 1, V3: 1, V4: 1, V5: 1, V6: 1, V7: 1,
    S0: 1, S1: 1, S2: 1, S3: 1, S4: 1, S5: 1,
    RET: 1, FP: 1, SP: 1, LR: 1,
    A0: 1, A1: 1, A2: 1, A3: 1, A4: 1, A5: 1,
};

// [m120] compiler/core/types.js Type 枚举键→串值。成员读折成串字面量。
const HOT_TYPE_CONST = {
    UNKNOWN: "unknown", NUMBER: "number", STRING: "string", BOOLEAN: "boolean",
    NULL: "null", UNDEFINED: "undefined", ARRAY: "array", OBJECT: "object",
    FUNCTION: "function", DATE: "Date", MAP: "Map", SET: "Set", REGEXP: "RegExp",
    TYPED_ARRAY: "TypedArray", ARRAY_BUFFER: "ArrayBuffer", DATA_VIEW: "DataView",
    BIGINT: "bigint", VOID: "void",
...
const ArefMethodRef = {
    ...
    array: {
        ...
        splice: ["_agen_splice", 2],
        sort: ["_agen_sort", 1],
        values: ["_agen_values", 0],
        keys: ["_agen_keys", 0],
        entries: ["_agen_entries", 0],
        toReversed: ["_agen_toReversed", 0],
        toSorted: ["_agen_toSorted", 1],
        toSpliced: ["_agen_toSpliced", 2],
        with: ["_agen_with", 2],
        // [I5 补] 缺失的一等值方法(与 ARRAY_PROTO_METHODS 同 helper/arity)。
        // includes 的 fromIndex 装箱(缺参 undefined 合法);join 委托 _agen_join;
        // toString 须动态 Get("join")，不可调用时回退 Object.prototype.toString；
        // shift/unshift 无下标参数、直接透传。
        includes: ["_agen_includes", 1],
        join: ["_agen_join", 1],
        toString: ["_agen_toString", 0],
        toLocaleString: ["_agen_toLocaleString", 0],
    ...
    string: {
        ...
        // _str_search returns a raw integer for its internal callers; the
        // method-value/boxed-wrapper ABI must expose a Number value.
        search: ["_aref_str_search", 1],     // [L3]
...
const STRING_PROTO_METHODS = [
    ...
    ["codePointAt", "_str_proto_codePointAt_utf16", 1],
    ["trim", "_str_trim", 0],
...
// BigInt.prototype 自有方法(遮蔽 Object.prototype.toLocaleString 继承链)。
const BIGINT_PROTO_METHODS = [
    ["toString", "_aref_bi_toString", 1],
    ["toLocaleString", "_aref_bi_toLocaleString", 0],
    ["valueOf", "_aref_bi_valueOf", 0],
];
...
// [gOPD 补全] max 以合成体收录(体内 Math.max(a,b) 走 compileMathMethod 同一内联快路;
// 规范 length=2;>2 实参/0 实参边角记偏差 —— 合成体只吃前两参)。
// 仍未收录(需新 helper):f16round/sumPrecise。
// @@toStringTag 已由 emitMathNamespaceObject → emitWellKnownToStringTag 落地。
const MATH_NS_SYNTH = {
    sign: { arity: 1 },
    imul: { arity: 2 },
    max: { arity: 2 },
...
const BUILTIN_REF_ARITY = {
    ...
    math_fround: 1, math_f16round: 1, math_clz32: 1,
    ...
    object_getOwnPropertyDescriptors: 1,
    ...
    fnproto_call: 1, fnproto_apply: 2, fnproto_bind: 1,
    ...
    promise_allSettled: 1, promise_any: 1, promise_withResolvers: 0, promise_try: 1,
    proxy_revocable: 2,
    ...
    // [ES2025] RegExp.escape 静态(emitRegExpCtorObject / 值读同槽)
    re_escape: 1,
    // [test262] $262.detachArrayBuffer → __detachArrayBuffer → _arraybuffer_detach
    host_detach_ab: 1,
};
...
// 收录 Error 族(含 AggregateError；其 Promise.any 运行时也复用同一 memoized 槽)。
// fnptr 用占位 _object_new
...
function _mkNameSet(names) {
    const o = {};
    for (let i = 0; i < names.length; i++) o[names[i]] = 1;
    return o;
}
const ERR_CTOR_NAME_SET = _mkNameSet(ERR_CTOR_NAMES);
...
// @@toStringTag 已由 emitCollectionCtorObject → emitWellKnownToStringTag 落在
// Map/Set/Promise.prototype(Array 走 IsArray builtinTag,不挂);Symbol.species 已落。
// [底层A Array 一等值] Array.prototype 方法值表(_agen_* 泛型 helper,供
// emitArrayCtorObject 物化原型;`Array.prototype.<m>` 值读经单例原型对象 _object_get
// 取同一 own prop,与 gOPD 恒等——见 compileMemberExpression 的 [test262 S1] 分支)。
// [gOPD 补全] toString 使用 _agen_toString：动态 Get(this,"join")，可调用则
// Call(join,this)，否则调用 %Object.prototype.toString%，覆盖原语/Proxy/覆写 join。
// 缺 unshift/toLocaleString 等——列为后续增量。
const ARRAY_PROTO_METHODS = [
    ...
    ["with", "_agen_with", 2],
    ["toString", "_agen_toString", 0],
    // 记偏差:元素级 toLocaleString 未做;先走 join 泛型(含 ToObject)堵 nullish 抛错面
    ["toLocaleString", "_agen_toLocaleString", 0],
];
const ARRAY_PROTO_METHODS_REF = {};
for (let _ai = 0; _ai < ARRAY_PROTO_METHODS.length; _ai++) {
    const _a = ARRAY_PROTO_METHODS[_ai];
    ARRAY_PROTO_METHODS_REF[_a[0]] = { label: _a[1], arity: _a[2] };
}
...
//                   → Annex B legacy(经 _object_define_property / gOPD 合成)
...
const OBJECT_PROTO_METHODS = [
    ...
    ["toLocaleString", "_object_proto_toLocaleString", 0],
    ["isPrototypeOf", "_is_prototype_of", 1],
    ["propertyIsEnumerable", "_object_propertyIsEnumerable", 1],
    ["__defineGetter__", "_aref_obj_defineGetter", 2],
    ["__defineSetter__", "_aref_obj_defineSetter", 2],
    ["__lookupGetter__", "_aref_obj_lookupGetter", 1],
    ["__lookupSetter__", "_aref_obj_lookupSetter", 1],
];
...
const OBJECT_STATIC_METHODS = [
    ...
    ["getOwnPropertyDescriptors", "_object_getOwnPropertyDescriptors", 1],
...
const MAP_PROTO_METHODS = [
    ...
    ["getOrInsert", "_aref_map_getOrInsert", 2],
    ["getOrInsertComputed", "_aref_map_getOrInsertComputed", 2],
];
...
const SET_PROTO_METHODS = [
    ...
    ["union", "_aref_set_union", 1],
    ["intersection", "_aref_set_intersection", 1],
    ["difference", "_aref_set_difference", 1],
    ["symmetricDifference", "_aref_set_symdiff", 1],
    ["isSubsetOf", "_aref_set_issubset", 1],
...
const IDENT_BUILTIN_NAMES = [
    ...
    "Boolean", "Number", "String", "Symbol", "JSON", "Reflect", "print",
    // [W7-3] 裸名物化为真闭包(parse* 与 Number.parse* 同槽;isNaN/isFinite 合成函数)。
    "parseInt", "parseFloat", "isNaN", "isFinite",
];
const IDENT_BUILTIN_NAME_SET = _mkNameSet(IDENT_BUILTIN_NAMES);
...
const IDENT_KNOWN_GLOBAL_NAME_SET = _mkNameSet(IDENT_KNOWN_GLOBAL_NAMES);
...
export const MemberCompiler = {
    ...
    emitPrivateBrandCheck(mangledName, mode) {
        ...
        // Legacy/non-method call sites use zero and retain the old own-key
        // check.  Keep this value in A3: all private checks happen before the
        // subsequent property read and A3 is otherwise scratch here.
        // A static method's `this` is a class object while an instance private
        // access may target an ordinary instance (`obj?.#x`); feeding that
    ...
    emitObjectGetIC(propName) {
        ...
        // 同函数内同 prop 复用一个站点槽:少 lea/adrp fixup 与 data label。
        // 必须用 Map:gen1 上 {} 动态键走线性 _object_get(见 CompileContext.locals)。
        if (!this._icGetByProp) this._icGetByProp = new Map();
        let siteLabel = this._icGetByProp.get(propName);
        if (!siteLabel) {
            siteLabel = this.ctx.newLabel("icg_site");
            this._icGetByProp.set(propName, siteLabel);
    ...
    // 严格模式走 _object_set_strict（IC 快路不查 writable，且 PutValue 须抛 TypeError）。
    emitObjectSetIC(propName) {
        ...
        if (this._envNoIC || this.engineNoIC || strictSet) {
            this.emitBoxedStringKey(propName, VReg.A1);
            vm.call(strictSet ? "_object_set_strict" : "_object_set");
            return;
        }
        if (!this._icSetByProp) this._icSetByProp = new Map();
        let siteLabel = this._icSetByProp.get(propName);
        if (!siteLabel) {
            siteLabel = this.ctx.newLabel("ics_site");
            this._icSetByProp.set(propName, siteLabel);
    ...
    emitMathNamespaceObject() {
        ...
        // [W11 Desc] sign/imul/max/min/hypot/random 合成一等值(无单一 helper;调用快路仍内联)
        for (const mname of Object.keys(MATH_NS_SYNTH)) {
            ...
            emitProp(mname, BUILTIN_PROP_ATTR, () => {
                const ast = mname === "sign" ? this._mathSignSynthAst()
                    : mname === "imul" ? this._mathImulSynthAst()
                    : mname === "min" ? this._mathMinSynthAst()
                    : mname === "hypot" ? this._mathHypotSynthAst()
                    : mname === "random" ? this._mathRandomSynthAst()
                    : mname === "sumPrecise" ? this._mathSumPreciseSynthAst()
                    : this._mathMaxSynthAst();
                this.emitSynthStaticRef("math_" + mname, ast, mname, arity, true);
            });
        }
        // Math[@@toStringTag]="Math"(attr 4)→ Object.prototype.toString.call(Math)
        // === "[object Math]"(Array.every.call(Math) 等靠品牌串辨认接收者)。
        this.emitWellKnownToStringTag(slot, "Math");
        vm.lea(VReg.V0, slot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.label(doneL);
    },
    ...
    // [Reflect 一等值] `Reflect` 标识符被遮蔽?同 jsonNameShadowed 守卫组。
    reflectNameShadowed() {
        return !!((this.ctx.getLocal && this.ctx.getLocal("Reflect")) ||
            (this.ctx.getFunction && this.ctx.getFunction("Reflect")) ||
            (this.ctx.getMainCapturedVar && this.ctx.getMainCapturedVar("Reflect")));
    },
    ...
    numberShimReady() {
        ...
        if (!this.getFunctionLabel("__NUM_toFixed")) return false;
    ...
    _tryEmitRegExpMethodRef(expr, propName) {
        ...
        // `toString` is frequently replaced as an own property (and the
        // Object.prototype.toString S15.10 brand test does exactly that).
        // A static RegExp method-value closure would bypass that override;
        // leave this one method to the ordinary property lookup path, which
        // still resolves RegExp.prototype.toString when unmodified.
        if (propName === "toString") return false;
    ...
    emitRegExpMethodClosure(methodName, shimName, arity) {
        ...
        // `_aref_generic` is shared by all built-in method closures and has a
        // metadata fallback name/arity (empty/0).  Assignment-mode
        // `_closure_prop_set` therefore sees name/length as already present
        // and silently ignores the initialization.  DefineOwnProperty is the
        // correct materialization path, just as in
        // `emitBuiltinMethodRefClosureMeta`, and records the per-closure value.
    ...
    // Object.prototype.hasOwnProperty.call(this, propName) —— 只认自有槽,不触发
    // 原型访问器(子类实例无 __RE_new 标志槽时否则会无限递归)。
    _reHasOwnCallAst(propName) {
        // Each occurrence must be a distinct AST node.  The compiler annotates
        // member nodes while lowering (and some fast paths replace them in
        // place); sharing one `Object.prototype` node between the callee and
        // the first argument made the generated RegExp accessors mutate one
        // occurrence while compiling the other.  The resulting getter looked
        // callable in reflection but jumped through a non-function at runtime
        // for `get.call({ ... })`.
        const makeObjProto = () => ({
            type: "MemberExpression",
        ...
        const objProtoForCallee = makeObjProto();
        const objProtoForHasOwn = makeObjProto();
        return {
            type: "CallExpression",
            callee: {
                type: "MemberExpression",
                object: objProtoForCallee,
    ...
    _reRegExpDefaultLit(defaultLit) {
        // `null` in REGEXP_PROTO_ACCESSORS denotes the absence of an
        // [[OriginalFlags]] slot, not the boolean value false.  The spec's
        // RegExp.prototype getter returns `undefined` for that receiver (and
        // for an object without the corresponding own slot); conflating the
        // sentinel with false made `get.call(RegExp.prototype)` observable as
        // `false` for global/multiline/etc.
        if (defaultLit === null) return { type: "Identifier", name: "undefined" };
        return { type: "Literal", value: defaultLit };
    },

    _regexpGetterAst(flagName, defaultLit) {
        // %RegExp.prototype% is materialized with a private __reProto marker,
        // but the full receiver/brand semantics are best kept in the shim:
        // `flags` accepts arbitrary objects and observes getter order, while
        // each individual accessor requires an actual RegExp instance.  A
        // single helper call also avoids AST node sharing/mutation in this
        // synthetic function body.
        const helper = flagName === "flags" ? "__RE_proto_flags" : "__RE_proto_flag";
        const helperArgs = flagName === "flags"
            ? [{ type: "ThisExpression" }]
            : [{ type: "ThisExpression" }, { type: "Literal", value: flagName }];
        return {
            type: "FunctionExpression",
            id: null,
            params: [],
            _fnHint: "get " + flagName,
            body: {
                type: "BlockStatement",
                body: [
                    {
    ...
    _reEnsureSlot(slotLabel) {
        ...
        const dataLabelsArr = this.asm.dataLabels || [];
        for (let i = 0; i < dataLabelsArr.length; i = i + 1) {
            if (dataLabelsArr[i].name === slotLabel) return;
        }
    ...
    _reSetProtoProp(protoSlot, name, attr) {
        const vm = this.vm;
        // x64: value 不得活在 RET/A2 上跨 lea/_tag_key_a1。帧槽重载 A2
        // (gOPN 已见键但 Get/gOPD 丢失 → 值曾被写成 0)。
        const stash = this.ctx.allocLocal("__reprop_" + this.nextLabelId());
        vm.store(VReg.FP, stash, VReg.RET);
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey(name, VReg.A1);
        vm.load(VReg.A2, VReg.FP, stash);
        vm.call("_object_set");
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey(name, VReg.A1);
    ...
    // [L1 @@toStringTag] 在装箱对象槽上落 well-known Symbol.toStringTag=tag
    // (attr 4 = {w:false,e:false,c:true},规范内建品牌串;可 delete → 回落 builtinTag)。
    // 形态镜像 JSON 命名空间(emitJSONNamespaceObject):well-known 单例作键 +
    // _object_set 落值 + _object_set_prop_attr 落 attrs。供 Math / Map·Set·Promise /
    // Date / RegExp 原型等缺 tag 的内建补齐 → Object.prototype.toString 得
    // "[object <tag>]"（Math 等无类型字节短路时尤关键）。
    emitWellKnownToStringTag(slot, tag) {
        const vm = this.vm;
        ...
        vm.lea(VReg.A2, this.asm.addString(tag));
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A2, VReg.A2, VReg.V1);
        vm.call("_object_set");
        vm.lea(VReg.V0, slot);
        vm.load(VReg.A0, VReg.V0, 0);
        vm.mov(VReg.A1, VReg.S0);
        vm.movImm(VReg.A2, ACCESSOR_PROP_ATTR);        // {w:false,e:false,c:true}
        vm.call("_object_set_prop_attr");
    },

    // [gOPD 补全] RegExp shim 未注入单元的原型方法**合成占位**体(反射用;记偏差):
    // 体一律 `throw new TypeError`——规范这些方法对非 RegExp brand 的 this 必须抛
    // TypeError(S15.10.6.2_A2_T6/T10、6.3_A2_T4:提取后 `__instance.exec(...)` 期望
    // TypeError,早前占位返回值吞了异常 → 回退)。无 shim 的编译单元不含正则字面量/
    // 构造文本(有则 index.js 必注入 shim)→ 造不出正则实例,brand 恒不满足,
    // 无条件抛与"查 brand 后抛"语义等价且更省。gOPD 簇(-166)只读描述符不调用,
    // 不受影响。
    _rePlaceholderSynthAst(name) {
        ...
        const throwTE = () => ({
            ...
            body: [{ type: "ThrowStatement", argument: {
                ...
                arguments: [{ type: "Literal",
                    value: "Method RegExp.prototype." + name + " called on incompatible receiver" }],
            } }],
        });
        // 形参个数须与规范 length 一致:合成函数入函数元数据侧表(按 code_ptr),
        // _closure_prop_get 的 length 回落读元数据 arity → 形参不齐则 .length 读错。
        let body;
        let params;
        if (name === "test") { body = throwTE(); params = [id("s")]; }
        else if (name === "exec") { body = throwTE(); params = [id("s")]; }
        else if (name === "toString") { body = throwTE(); params = []; }
        else if (name === "escape") { body = throwTE(); params = [id("str")]; }
        else { body = throwTE(); params = [id("p"), id("f")]; } // compile
        return { type: "FunctionExpression", id: null, params: params, body: body };
    },
    ...
    emitRegExpCtorObject() {
        ...
        vm.store(VReg.V1, 0, VReg.RET);
        // 标志访问器认 %RegExp.prototype%:不可在 getter AST 写 RegExp.prototype
        // (编译期重入)。内部键 attr=0,不进 Object.keys。
        vm.movImm(VReg.RET, 1);
        vm.scvtf(0, VReg.RET);
        vm.fmovToInt(VReg.RET, 0);
        this._reSetProtoProp(protoSlot, "__reProto", 0);
        // 原型属性落位:先 _object_set 落值,再 _object_set_prop_attr 落 attrs
        // (顺序不可反,同 emitMathNamespaceObject 注)。值先落 RET,再调 _reSetProtoProp。
        // [gOPD 补全] shim 未注入(源码无正则字面量/构造文本,如仅反射
        // `gOPD(RegExp.prototype,"test")`,15.2.3.3-4-166)时不再跳过:改挂**合成占位**
        // 方法(memoized,name/length/attr 规范)——test 恒 false、exec 恒 null、
        // toString 恒 "/(?:)/"、compile 返 this(记偏差:占位不做匹配;无 shim 的单元
        // 造不出正则实例,占位只可能被反射/泛型形态触达)。占位口径同
        // DATE_PROTO_SHIM_METHODS / ERR_CTOR_PLACEHOLDER_FN 先例。
        for (let i = 0; i < REGEXP_PROTO_METHODS.length; i = i + 1) {
            const m = REGEXP_PROTO_METHODS[i];
            if (this.getFunctionLabel(m[1])) {
                this.emitRegExpMethodClosure(m[0], m[1], m[2]); // RET = 方法值
            } else {
                // wrapNonCtor:24B {magic,_aref_static_tramp,fnptr} → `new` 走
                // compileDynamicNew 的 tramp 检查抛 TypeError(S15.10.6.4_A7:
                // RegExp.prototype.toString 不可 new);调用经蹦床进占位体照常抛。
                this.emitSynthStaticRef("re_" + m[0],
                    this._rePlaceholderSynthAst(m[0]), m[0], m[2], true);
            }
        ...
        // RegExp.prototype[@@toStringTag]="RegExp"(反射/delete 面;实例 toString 仍可
        // 走 __isRegExp 探测)。
        // RegExp.prototype = 原型对象(闭包属性侧表)。[gOPD 补全] 落 attr 0:规范
        // {writable:false, enumerable:false, configurable:false}(15.2.3.3-4-211;
        // 此前不落 attr → 侧表默认 7,gOPD(RegExp,"prototype") 误报全真)。
        vm.lea(VReg.V0, ctorSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        ...
        // [ES2025] RegExp.escape：构造器自有数据属性。即使当前编译单元
        // 没有直接调用 RegExp.escape（例如仅做 gOPD/verifyProperty），也
        // 必须暴露一个不可构造的函数形状；有 shim 时复用真实实现，否则
        // 用占位体保留 own-property/name/length/attrs 反射契约。
        const reEscLabel = this.getFunctionLabel("__RE_escape");
        if (reEscLabel) {
            this.emitMemoizedBuiltinRef("re_escape", reEscLabel, "escape");
            vm.mov(VReg.A2, VReg.RET);
            vm.lea(VReg.V0, ctorSlot);
            vm.load(VReg.A0, VReg.V0, 0);
            this.emitBoxedStringKey("escape", VReg.A1);
            vm.call("_closure_prop_set");
            vm.lea(VReg.V0, ctorSlot);
            vm.load(VReg.A0, VReg.V0, 0);
            this.emitBoxedStringKey("escape", VReg.A1);
            ...
            this.emitSynthStaticRef("re_escape", this._rePlaceholderSynthAst("escape"),
                "escape", 1, true);
            vm.mov(VReg.A2, VReg.RET);
            vm.lea(VReg.V0, ctorSlot);
            vm.load(VReg.A0, VReg.V0, 0);
    ...
    emitStringCtorObject() {
        ...
        vm.call("_closure_prop_set_attr");
        // 原型对象:若 _ensure_string_proto 已填槽(new String 早于 String 值读),复用
        // 同对象,避免实例 __proto__ 与 String.prototype 身份分裂 → instanceof 恒假
        // (Object.create Properties 为 String 包装,15.2.3.5-4-7)。
        const protoReadyL = this.ctx.newLabel("nsstr_proto_ready");
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.cmpImm(VReg.RET, 0);
        vm.jne(protoReadyL);
        ...
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        vm.lea(VReg.A1, this.asm.addString("__value"));
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A1, VReg.A1, VReg.V1);
        vm.lea(VReg.A2, this.asm.addString(""));
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A2, VReg.A2, VReg.V1);
        vm.call("_object_set");
        vm.lea(VReg.V0, protoSlot);
        ...
        for (let i = 0; i < STRING_PROTO_METHODS.length; i = i + 1) {
            ...
            // vm.lea 在链接期解析为运行时函数地址。replace/replaceAll
            // additionally have a full String.prototype wrapper in the RegExp
            // shim. Use it when that shim is present so method-value calls
            // (`String.prototype.replaceAll.call(...)`) still perform
            // IsRegExp/GetMethod/flags checks; the low-level helpers bypass
            // those observable steps. Keep the legacy helper when no shim is
            // linked (compiler/runtime-only modules).
            let helperLabel = m[1];
            if (m[0] === "replace" || m[0] === "replaceAll") {
                const fullName = m[0] === "replace" ? "__RE_string_replace" : "__RE_string_replaceAll";
                const fullLabel = this.getFunctionLabel && this.getFunctionLabel(fullName);
                if (fullLabel) helperLabel = fullLabel;
            }
            // Method-value calls through `String.prototype.<name>` must keep
            // the RegExp protocol wrapper when available.  The low-level
            // helpers are intentionally retained for bootstrap units without
            // the shim, but would otherwise leak boxed receivers and skip
            // GetMethod/RegExpCreate for match/matchAll/search.
            if (m[0] === "match" || m[0] === "matchAll" || m[0] === "search") {
                const fullName = m[0] === "match" ? "__RE_string_match" :
                    m[0] === "matchAll" ? "__RE_string_matchAll" : "__RE_string_search";
                const fullLabel = this.getFunctionLabel && this.getFunctionLabel(fullName);
                if (fullLabel) helperLabel = fullLabel;
            }
            this.emitBuiltinMethodRefClosure(helperLabel); // RET = 闭包
            // 挂 .name/.length:须 _closure_prop_define(元数据 arity=0 时 _closure_prop_set 静默忽略 length)
            vm.mov(VReg.S0, VReg.RET);
            vm.mov(VReg.A0, VReg.S0);
            this.emitBoxedStringKey("name", VReg.A1);
        ...
        vm.load(VReg.RET, VReg.V0, 0);
        this._reSetProtoProp(protoSlot, "constructor", BUILTIN_PROP_ATTR);
        // String.prototype[@@iterator]: 挂字符串键与 well-known Symbol 键
        this.emitBuiltinMethodRefClosureMeta("_str_iterator", "[Symbol.iterator]", 0);
        this._reSetProtoProp(protoSlot, "Symbol.iterator", BUILTIN_PROP_ATTR);
        vm.lea(VReg.A0, "_symwk_iterator");
        vm.lea(VReg.A1, this.asm.addString("Symbol.iterator"));
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A1, VReg.A1, VReg.V1);
        vm.call("_symbol_wellknown");
        const strSymSlot = this.ctx.allocLocal("__str_sym_" + this.nextLabelId());
        vm.store(VReg.FP, strSymSlot, VReg.RET);
        this.emitBuiltinMethodRefClosureMeta("_str_iterator", "[Symbol.iterator]", 0);
        const strFnSlot = this.ctx.allocLocal("__str_fn_" + this.nextLabelId());
        vm.store(VReg.FP, strFnSlot, VReg.RET);
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        vm.load(VReg.A1, VReg.FP, strSymSlot);
        vm.load(VReg.A2, VReg.FP, strFnSlot);
        vm.call("_object_set");
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        vm.load(VReg.A1, VReg.FP, strSymSlot);
    ...
    // [W-29] `String.prototype` 值读:始终经构造器物化,避免 `_ensure_string_proto`
    // 已填槽时早退导致 constructor 未回指(同 Boolean S15.6.2.1_A2)。
    ...
    // [gOPD 补全] call/apply 挂为 own prop(attr 5),值经 emitMemoizedBuiltinRef
    // 与静态链 `Function.prototype.call/apply` 取值特判**同槽**(fnproto_call/fnproto_apply)
    // → gOPD(Function.prototype,"apply").value === Function.prototype.apply 恒等。
    // 静态链取值特判仍先于本路径命中(读槽,不读对象)→ 其字节序列不变。
    // bind 经 _fp_bind_tramp 挂 own(与 call/apply 同槽 fnproto_bind)→ 4-611 hasOwn。
    ...
    emitFunctionProtoObject() {
        ...
        const fillL = this.ctx.newLabel("nsfnproto_fill");
        const bindL = this.ctx.newLabel("nsfnproto_bind");
        const toStringL = this.ctx.newLabel("nsfnproto_tostring");
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.cmpImm(VReg.RET, 0);
        vm.jne(fillL);
        ...
        vm.store(VReg.V1, 0, VReg.RET);
        vm.label(fillL);
        // `_ensure_function_proto` 可能已先填空单例;槽非空时仍须补挂 call/apply
        // (否则 gOPD(Function.prototype,"apply") 得 undefined,15.2.3.3-4-36)。
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("apply", VReg.A1);
        vm.call("_object_has");
        vm.cmpImm(VReg.RET, 0);
        vm.jne(bindL);
        this.emitMemoizedBuiltinRef("fnproto_call", "_fp_call_tramp", "call");
        ...
        vm.label(bindL);
        // apply 已在的早退不得跳过 bind(旧物化缺 bind → 4-611 hasOwn 失败)。
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("bind", VReg.A1);
        vm.call("_object_has");
        vm.cmpImm(VReg.RET, 0);
        vm.jne(doneL);
        this.emitMemoizedBuiltinRef("fnproto_bind", "_fp_bind_tramp", "bind");
        this._reSetProtoProp(protoSlot, "bind", BUILTIN_PROP_ATTR);
        vm.label(doneL);
        // Function.prototype.toString is an own method. _fp_toString consumes
        // its receiver, so materialize it as a generic method reference.
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        ...
        vm.cmpImm(VReg.RET, 0);
        vm.jne(toStringL);
        this.emitMemoizedBuiltinMethodRef("fnproto_toString", "_fp_toString", "toString", 0);
        this._reSetProtoProp(protoSlot, "toString", BUILTIN_PROP_ATTR);
        vm.label(toStringL);
        // [test262] caller / arguments poison pills on %FunctionPrototype%.
        // Previously installed only in the apply-miss path; `_ensure_function_proto`
        // fills apply first → Class.caller/arguments inherited miss (undefined)
        // instead of %ThrowTypeError%. Check independently of apply/bind.
        const throwAccL = this.ctx.newLabel("nsfnproto_throwacc");
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("caller", VReg.A1);
        vm.call("_object_has");
        vm.cmpImm(VReg.RET, 0);
        vm.jne(throwAccL);
        ...
        vm.label(throwAccL);
        // Ensure Function.prototype.constructor and the constructor's own
        // prototype/name/length descriptors without recursively rebuilding
        // this already-created prototype.
        this.emitFunctionCtorObject(true);
        // 调用方期望 RET = boxed Function.prototype(含 apply 已存在的早退路径)。
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.RET, VReg.V0, 0);
    },
    ...
    // [gOPD 补全] 裸 `Function` 一等值合成体:动态 `new F()`/`F()`(F===Function)返回
    // 空函数对象——供 Object.seal(new (Object.getPrototypeOf(()=>{}).constructor)())
    // 等反射面。带源码的 `new Function(...)/Function(...)` 语法位仍改派 __makeFunction。
    // 形参 1 个对齐规范 Function.length=1。
    _functionCtorSynthAst() {
        const id = (n) => ({ type: "Identifier", name: n });
        return {
            type: "FunctionExpression",
            id: null,
    ...
    // [gOPD 补全] 裸 `Function`(值位/反射位)→ 惰性物化单例真闭包。槽
    // _fnctor_singleton 由 allocator.js 数据段**常驻定义**(GC 根扫描区;不走
    // 编译期 addDataLabel:jsvalue.js _instanceof 无条件引用,按需发射会缺符号)。
    // 侧表:name="Function"/length=1(attr 4,ES 20.2.1)+ prototype =
    // Function.prototype 单例(_nsobj_function_proto 同槽,attr 0 规范全 false,
    // 15.2.3.3-4-185)→ gOPD(Function,"prototype").value === Function.prototype 恒等。
    // typeof Function 由(旧哨兵的)"number" 转正为 "function",顺带收益。
    emitFunctionCtorObject(prototypeAlreadyEnsured = false) {
        const vm = this.vm;
        const slot = "_fnctor_singleton";
        const doneL = this.ctx.newLabel("nsfnctor_done");
        vm.lea(VReg.V0, slot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.cmpImm(VReg.RET, 0);
        vm.jne(doneL);
        this.compileFunctionExpression(this._functionCtorSynthAst()); // RET = 装箱闭包
        vm.lea(VReg.V1, slot);
        vm.store(VReg.V1, 0, VReg.RET);
        vm.mov(VReg.S0, VReg.RET);           // 跨 call 暂存(同 emitSynthStaticRef)
        vm.mov(VReg.A0, VReg.S0);
        this.emitBoxedStringKey("name", VReg.A1);
        vm.lea(VReg.A2, this.asm.addString("Function"));
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A2, VReg.A2, VReg.V1);
        vm.call("_closure_prop_set");
        vm.mov(VReg.A0, VReg.S0);
        this.emitBoxedStringKey("name", VReg.A1);
        vm.movImm(VReg.A2, 4); // {w:0,e:0,c:1}
        vm.call("_closure_prop_set_attr");
        vm.mov(VReg.A0, VReg.S0);
        this.emitBoxedStringKey("length", VReg.A1);
        vm.movImm(VReg.A2, 1);
        ...
        vm.movImm(VReg.A2, 4);
        vm.call("_closure_prop_set_attr");
        // prototype 键:emitFunctionProtoObject 内部的 emitMemoizedBuiltinRef 会
        // 覆写 S0/S1(发射器局部约定,非 call 保存)→ ctor/proto 一律从各自槽重载,
        // 不依赖跨发射器的寄存器存活。
        if (!prototypeAlreadyEnsured) {
            this.emitFunctionProtoObject();  // 填 _nsobj_function_proto(单例)
        }
        vm.lea(VReg.V0, slot);
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("prototype", VReg.A1);
        ...
        vm.movImm(VReg.A2, BUILTIN_CONST_ATTR); // 规范全 false
        vm.call("_closure_prop_set_attr");
        // Function.prototype.constructor = Function(seal-arrowfunction /
        // getPrototypeOf(fn).constructor === Function)
        vm.lea(VReg.V0, slot);
        vm.load(VReg.RET, VReg.V0, 0);
        this._reSetProtoProp("_nsobj_function_proto", "constructor", BUILTIN_PROP_ATTR);
        vm.lea(VReg.V0, slot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.label(doneL);
    },
    ...
    // 必须走 emitBooleanCtorObject: `_ensure_boolean_proto`(new Boolean 路径)可能已
    // 填 proto 槽但未挂 constructor/Boolean.prototype。若此处见槽非空就早退,
    // `Boolean.prototype === x.constructor.prototype` 会变成 A !== Object.prototype
    // (test262 S15.6.2.1_A2)。
    ...
    emitBooleanCtorObject() {
        ...
        // 若 _ensure_boolean_proto 已填槽(new Boolean 早于 Boolean 值读),复用同对象,
        // 避免实例 __proto__ 与 Boolean.prototype 身份分裂。
        const protoReadyL = this.ctx.newLabel("nsbool_proto_ready");
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.cmpImm(VReg.RET, 0);
        vm.jne(protoReadyL);
        ...
        for (let i = 0; i < BOOLEAN_PROTO_METHODS.length; i++) {
            ...
            vm.mov(VReg.S0, VReg.RET);
            // name/length:须 _closure_prop_define(_closure_prop_set 对已有元数据 name 静默忽略)
            vm.mov(VReg.A0, VReg.S0);
            this.emitBoxedStringKey("name", VReg.A1);
            vm.lea(VReg.A2, this.asm.addString(m[0]));
            vm.movImm64(VReg.V1, 0x7ffc000000000000n);
    ...
    emitSymbolCtorObject() {
        ...
        vm.fmovToInt(VReg.A2, 0);
        vm.call("_closure_prop_set");
        // Prototype object: reuse _ensure_symbol_proto if ToObject(Symbol)
        // ran before the Symbol identifier (sort.call(Symbol()) instanceof Symbol).
        const protoReadyL = this.ctx.newLabel("nssym_proto_ready");
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.cmpImm(VReg.RET, 0);
        vm.jne(protoReadyL);
        ...
        // 须 Meta(= _closure_prop_define):_aref_generic 元数据 arity=0/name="" 会令
        // _closure_prop_set 把 name/length 当「已存在不可写」静默忽略。
        const SYM_PROTO = [["toString", "_symbol_to_string", 0], ["valueOf", "_symbol_valueOf", 0]];
        for (let i = 0; i < SYM_PROTO.length; i++) {
            const m = SYM_PROTO[i];
            this.emitBuiltinMethodRefClosureMeta(m[1], m[0], m[2]);
            this._reSetProtoProp(protoSlot, m[0], BUILTIN_PROP_ATTR);
        }
        // Symbol.prototype.description accessor (get description)
        this.emitBuiltinMethodRefClosureMeta("_symbol_description", "get description", 0);
        vm.mov(VReg.S0, VReg.RET);
        vm.movImm(VReg.A0, 24);
        vm.call("_alloc");
        ...
        vm.movImm64(VReg.V1, PTR_MASK_BITS);
        vm.and(VReg.V2, VReg.V2, VReg.V1);
        vm.store(VReg.S1, 8, VReg.V2);
        vm.movImm(VReg.V1, 0);
        vm.store(VReg.S1, 16, VReg.V1);
        this.emitBoxedStringKey("description", VReg.A1);
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.V2, VReg.V0, 0);
        vm.movImm64(VReg.V1, PTR_MASK_BITS);
        vm.and(VReg.A0, VReg.V2, VReg.V1);
        ...
        vm.load(VReg.RET, VReg.V0, 0);
        this._reSetProtoProp(protoSlot, "constructor", BUILTIN_PROP_ATTR);
        this.emitWellKnownToStringTag(protoSlot, "Symbol");
        // Symbol.prototype[Symbol.toPrimitive] = function(hint){ return thisSymbolValue(this); }
        // 语义同 valueOf(忽略 hint);_aref_generic 把 this→A0、hint→A1,可复用 _symbol_valueOf。
        this.emitBuiltinMethodRefClosureMeta("_symbol_valueOf", "[Symbol.toPrimitive]", 1);
        vm.mov(VReg.S0, VReg.RET);
        ...
        const setSymProp = (name, helperLabel, arity) => {
            this.emitBuiltinMethodRefClosureMeta(helperLabel, name, arity);
            vm.mov(VReg.S0, VReg.RET);
            vm.mov(VReg.A2, VReg.S0);
            vm.lea(VReg.V0, ctorSlot);
            vm.load(VReg.A0, VReg.V0, 0);
        ...
        // The intrinsic is also the initial own value of globalThis.Symbol.
        vm.lea(VReg.V0, ctorSlot);
        vm.load(VReg.A2, VReg.V0, 0);
        vm.lea(VReg.V0, "_global_this");
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("Symbol", VReg.A1);
        vm.call("_object_set");
        vm.lea(VReg.V0, "_global_this");
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("Symbol", VReg.A1);
        vm.movImm(VReg.A2, 5); // writable:true, enumerable:false, configurable:true
        vm.call("_object_set_prop_attr");
    ...
    // [BigInt 一等值] BigInt 构造函数 + BigInt.prototype。
    // 供 instanceof BigInt / ToObject(bigint) 包装身份。调用快路 BigInt(x) 仍走 _to_bigint。
    emitBigIntCtorObject() {
        const vm = this.vm;
        const ctorSlot = "_nsobj_bigint";
        const protoSlot = "_nsobj_bigint_proto";
        this._reEnsureSlot(ctorSlot);
        this._reEnsureSlot(protoSlot);
        const doneL = this.ctx.newLabel("nsbi_done");
        const protoReadyL = this.ctx.newLabel("nsbi_proto_ready");
        const protoMethodsL = this.ctx.newLabel("nsbi_proto_methods");
        vm.lea(VReg.V0, ctorSlot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.cmpImm(VReg.RET, 0);
        vm.jne(protoReadyL);
        ...
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("name", VReg.A1);
        vm.lea(VReg.A2, this.asm.addString("BigInt"));
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A2, VReg.A2, VReg.V1);
        vm.call("_closure_prop_set");
        vm.lea(VReg.V0, ctorSlot);
        ...
        vm.load(VReg.RET, VReg.V0, 0);
        vm.cmpImm(VReg.RET, 0);
        vm.jne(protoMethodsL);
        vm.call("_object_new");
        vm.call("_box_obj_r");
        vm.lea(VReg.V1, protoSlot);
        vm.store(VReg.V1, 0, VReg.RET);
        vm.label(protoMethodsL);
        for (let i = 0; i < BIGINT_PROTO_METHODS.length; i = i + 1) {
            const m = BIGINT_PROTO_METHODS[i];
            this.emitBuiltinMethodRefClosureMeta(m[1], m[0], m[2]);
            this._reSetProtoProp(protoSlot, m[0], BUILTIN_PROP_ATTR);
        }
        vm.lea(VReg.V0, ctorSlot);
    ...
    // emitMemoizedBuiltinRef 同模式)。24B {magic, _aref_static_tramp, shim@16}:
    // shim 导出是普通编译函数(A0.. 装箱实参,不经 _aref_generic —— 蹦床会把 this
    // 插到 A0)。_aref_static_tramp 无接收者移位,值路径 s(v,r,sp) 与快路一致。
    // compileDynamicNew / IsConstructor(newTarget) 见 fnptr==_aref_static_tramp
    // → TypeError(rawJSON/parse/stringify/isRawJSON not-a-constructor)。
    ...
    emitJSONMethodRef(propName, arity) {
        ...
        vm.cmpImm(VReg.RET, 0);
        vm.jne(doneL);
        this.emitStaticBuiltinRefClosure(this.getFunctionLabel("__JSON_" + propName)); // RET = 24B non-ctor
    ...
    // [Reflect 一等值] 裸 `Reflect` 求值:惰性物化真命名空间对象到全局槽
    // _nsobj_reflect。此前 IDENT_KNOWN_GLOBAL 把 Reflect 当可解析、compileIdentifier
    // 兜底 movImm(RET,0) → typeof "number"(test262 typeof/built-in-ordinary-objects-no-call)。
    // 形态镜像 JSON 无 shim:空对象 + @@toStringTag="Reflect"。Reflect.* 调用快路
    // (functions.js 脱糖)先于本路径命中 → 快路字节不变。
    emitReflectNamespaceObject() {
        const vm = this.vm;
        const slot = "_nsobj_reflect";
        this._reEnsureSlot(slot);
        const doneL = this.ctx.newLabel("nsreflect_done");
        vm.lea(VReg.V0, slot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.cmpImm(VReg.RET, 0);
        vm.jne(doneL);
        ...
        vm.lea(VReg.V1, slot);
        vm.store(VReg.V1, 0, VReg.RET);
        this.emitWellKnownToStringTag(slot, "Reflect");
        vm.lea(VReg.V0, slot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.label(doneL);
    },
    ...
    // [gOPD 补全] Math.max 合成体:体内 `Math.max(a, b)` 由 compileCallExpression →
    // compileMathMethod 静态内联(与语法调用同一快路,NaN/±0 口径自动一致)。
    // 合成体固定二参(规范 length=2);变参调用(0/1/3+ 实参)记偏差 —— 一等值路径
    // 只服务反射(gOPD/gOPN/身份),语法调用 Math.max(...) 不经此。
    _mathMaxSynthAst() {
        const id = (name) => ({ type: "Identifier", name: name });
        return {
            type: "FunctionExpression",
    ...
    // Math.hypot 合成体:体内 `Math.hypot(a,b)` 走 compileMathHypot 快路(规范 length=2)。
    _mathHypotSynthAst() {
        const id = (name) => ({ type: "Identifier", name: name });
        return {
            type: "FunctionExpression",
    ...
    // Math.random 占位一等值(反射 name/length);语义调用未做熵源(记偏差,返 0)。
    _mathRandomSynthAst() {
        return {
            type: "FunctionExpression",
            id: null,
    ...
    // Math.sumPrecise(iterable):Shewchuk partials + overflow partial。有限项以
    // error-free TwoSum 展开;中间和越过 2**1024 时把超出部分记到
    // overflow(概念上乘 2**1024),避免大数抵消前提前溢出。最后按
    // round-to-nearest-ties-even 合并 partials。算法与 tc39/proposal-math-sum
    // 参考 polyfill 同形,但保留规范的非 Number 直接抛错(绝不 ToNumber),
    // 并遍历到尾部后再归纳 NaN/Infinity,使后续非 Number 仍能触发
    // TypeError。用本编译器自己的 parser 生成 AST,避免手写数百个易错节点。
    _mathSumPreciseSynthAst() {
        const program = this.parse(`
            var __sumPrecise = function (iterable) {
                if (arguments.length !== 1) {
                    throw new TypeError("Math.sumPrecise requires exactly one iterable");
                }
                ...
                var sawNaN = false;
                var allNegativeZero = true;

                for (var value of iterable) {
                    if (typeof value !== "number") {
                        throw new TypeError("Math.sumPrecise element must be a Number");
                    }
                    if (value !== value) {
                        sawNaN = true;
                    ...
                    if (!Object.is(value, -0)) allNegativeZero = false;
                    if (sawNaN || positiveInfinity || negativeInfinity) continue;

                    var x = value;
                    var used = 0;
                    for (var j = 0; j < partials.length; j = j + 1) {
                        var y = partials[j];
                        ...
                        var hi = x + y;
                        var lo = y - (hi - x);
                        if (hi === Infinity || hi === -Infinity) {
                            var sign = hi === Infinity ? 1 : -1;
                            overflow = overflow + sign;
                            if (Math.abs(overflow) >= 9007199254740992) {
                                throw new RangeError("Math.sumPrecise accumulator overflow");
                            }
                            x = (x - sign * 8.98846567431158e307) - sign * 8.98846567431158e307;
                            if (Math.abs(x) < Math.abs(y)) {
                                var swapOverflow = x;
                                x = y;
                                y = swapOverflow;
                ...
                if (sawNaN || (positiveInfinity && negativeInfinity)) return NaN;
                ...
                if (overflow !== 0) {
                    ...
                    if (Math.abs(overflow) > 1 ||
                        ((overflow > 0 && next > 0) || (overflow < 0 && next < 0))) {
                        return overflow > 0 ? Infinity : -Infinity;
                    }
                    hi = overflow * 8.98846567431158e307 + next / 2;
                    lo = next / 2 - (hi - overflow * 8.98846567431158e307);
                    lo = lo * 2;
                    if (hi * 2 === Infinity || hi * 2 === -Infinity) {
                        if (hi > 0) {
                            if (hi === 8.98846567431158e307 &&
                                lo === -(1.9958403095347198e292 / 2) &&
                                n >= 0 && partials[n] < 0) {
    ...
    emitNumberPredicateRef(propName) {
        this.emitSynthStaticRef("number_" + propName,
            this._numberPredicateSynthAst(propName), propName, 1, true);
    },
    ...
    emitNumberCtorObject() {
        ...
        // 本原型新建取 Node 序)。
        // 若 _ensure_number_proto 已填槽(_number_new / ToObject 早于 Number 值读),
        // 复用同对象,避免实例 __proto__ 与 Number.prototype 身份分裂
        // (15.4.4.4-5-c-i-1: concat.call(101) instanceof Number)。
        const protoReadyL = this.ctx.newLabel("nsnum_proto_ready");
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.cmpImm(VReg.RET, 0);
        vm.jne(protoReadyL);
        ...
        vm.store(VReg.V1, 0, VReg.RET);
        vm.label(protoReadyL);
        // Number.prototype 自身是 Number 对象,[[NumberData]]=+0(规范 20.1.3 /
        // S15.7.4_A1):挂 __number_value=+0,使 ==0 / ToNumber / valueOf 走包装路径,
        // 否则宽松相等对原型 SIGSEGV。
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        vm.lea(VReg.A1, this.asm.addString("__number_value"));
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A1, VReg.A1, VReg.V1);
    ...
    emitNumberProtoObject() {
        ...
        const protoSlot = "_nsobj_number_proto";
        this._reEnsureSlot(protoSlot);
        // 同 Boolean/String:勿因 proto 槽已填而跳过构造器物化。
    ...
    emitDateCtorObject() {
        ...
        vm.load(VReg.RET, VReg.V0, 0);
        this._reSetProtoProp(protoSlot, "constructor", BUILTIN_PROP_ATTR);
        // Date.prototype[@@toStringTag]="Date"(反射/delete 面;实例 toString 仍可走
        // TYPE_DATE 短路)。
    ...
    emitCollectionSizeAccessor(protoSlot, helperLabel) {
        ...
        vm.mov(VReg.A0, VReg.S0);
        this.emitBoxedStringKey("name", VReg.A1);
        // _tag_key_a1 会覆盖 A0/RET；键物化后必须恢复 getter 闭包接收者。
        vm.mov(VReg.A0, VReg.S0);
        vm.lea(VReg.A2, this.asm.addString("get size"));
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A2, VReg.A2, VReg.V1);
        // _aref_generic 已有 name="" 元数据且该属性不可写；DefineOwnProperty 才能
        // 为这一个 getter 闭包安装规范名 "get size"。
        vm.call("_closure_prop_define");
        vm.mov(VReg.A0, VReg.S0);
        this.emitBoxedStringKey("name", VReg.A1);
        vm.mov(VReg.A0, VReg.S0);
        vm.movImm(VReg.A2, 4); // configurable, not writable/enumerable
        vm.call("_closure_prop_set_attr");
        vm.mov(VReg.A0, VReg.S0);
        this.emitBoxedStringKey("length", VReg.A1);
        vm.mov(VReg.A0, VReg.S0);
    ...
    emitCollectionCtorObject(cfg) {
        ...
        const ctorHaveL = this.ctx.newLabel("nscoll_ctor_have");
        vm.lea(VReg.V0, ctorSlot);
        vm.load(VReg.RET, VReg.V0, 0);
        if (cfg.name === "Array") {
            // `_str_split` may adopt the process bootstrap's callable Array
            // into `_nsobj_array` before this site runs.  It is a lazy shell,
            // not a fully populated intrinsic; use the ready bit to continue
            // completion while preserving that constructor's identity.
            this._reEnsureSlot("_nsobj_array_ready");
            vm.lea(VReg.V1, "_nsobj_array_ready");
        ...
        if (cfg.name === "Array") {
            ...
            // Give it an explicit Object.prototype override through the existing
            // per-array prototype side table so array hole lookup never falls
            // back to the intrinsic array prototype (itself).
            vm.movImm(VReg.A0, 0);
            vm.call("_array_new_with_size");
            vm.call("_box_arr_r");
            vm.mov(VReg.S0, VReg.RET);
            // Do not publish the array as the intrinsic prototype until its
            // Object.prototype override is installed. Object.prototype lazy
            // materialisation can itself perform array operations; publishing
            // early makes a hole lookup recurse through this half-built array.
        ...
        // Array.prototype.length 初值 0,{W:true,E:false,C:false}。
        if (cfg.name === "Array") {
            vm.lea(VReg.V0, protoSlot);
            vm.load(VReg.A0, VReg.V0, 0);
            this.emitBoxedStringKey("length", VReg.A1);
            ...
            vm.load(VReg.A0, VReg.V0, 0);
            this.emitBoxedStringKey("length", VReg.A1);
            vm.movImm(VReg.A2, 1); // ATTR_WRITABLE only
            vm.call("_object_set_prop_attr");
        }
        // 原型方法落位:_aref_generic 安全 helper 闭包 + 逐闭包 .name/.length + attr 5。
        // [gOPD 补全] cfg.methodMemoPrefix(现仅 Array 传 "apm_"):闭包改经
        // emitMemoizedBuiltinMethodRef 建到全局槽 _builtinref_<prefix><名>,与
        // `Array.prototype.<m>` 值读分支同槽 → gOPD own prop 与值读身份恒等。
        for (let i = 0; i < cfg.methods.length; i = i + 1) {
            const m = cfg.methods[i];
            // Promise.prototype may already have been created by the runtime
            // when an instance method was read before the `Promise` identifier
            // itself.  Completing the constructor object must preserve those
            // existing method objects (and user overrides) rather than replace
            // them, otherwise `p.finally === Promise.prototype.finally` depends
            // on evaluation order.
            const methodDoneL = cfg.name === "Promise"
                ? this.ctx.newLabel("nscoll_method_done") : null;
            if (methodDoneL) {
                vm.lea(VReg.V0, protoSlot);
                vm.load(VReg.A0, VReg.V0, 0);
                this.emitBoxedStringKey(m[0], VReg.A1);
                vm.call("_object_has");
                vm.cmpImm(VReg.RET, 0);
                vm.jne(methodDoneL);
            }
            if (typeof cfg.methodMemoPrefix === "string") {
                this.emitMemoizedBuiltinMethodRef(cfg.methodMemoPrefix + m[0], m[1], m[0], m[2]);
            } else {
                this.emitBuiltinMethodRefClosureMeta(m[1], m[0], m[2]); // RET = 闭包
            }
            this._reSetProtoProp(protoSlot, m[0], BUILTIN_PROP_ATTR);
            if (methodDoneL) vm.label(methodDoneL);
        }
        ...
        // Array.prototype[@@iterator] = values。双键:
        // 1) 字符串 "Symbol.iterator"(for-of/spread/getMemberPropertyName 协议)
        // 2) well-known Symbol(gOPD / verifyProperty / getOwnPropertySymbols)
        if (cfg.name === "Array" && typeof cfg.speciesTmpSlot === "string") {
            const tmpSlot = cfg.speciesTmpSlot;
            this._reEnsureSlot(tmpSlot);
            vm.lea(VReg.V0, protoSlot);
            vm.load(VReg.A0, VReg.V0, 0);
            ...
            vm.or(VReg.A1, VReg.A1, VReg.V1);
            vm.call("_symbol_wellknown");
            vm.mov(VReg.S0, VReg.RET); // S0 = symbol 键(跨 call)
            vm.lea(VReg.V0, protoSlot);
            vm.load(VReg.A0, VReg.V0, 0);
            vm.mov(VReg.A1, VReg.S0);
            vm.lea(VReg.V0, tmpSlot);
            ...
            for (let ui = 0; ui < unscNames.length; ui = ui + 1) {
                vm.lea(VReg.V0, tmpSlot);
                vm.load(VReg.A0, VReg.V0, 0);
                this.emitBoxedStringKey(unscNames[ui], VReg.A1);
                vm.movImm64(VReg.A2, 0x7ff9000000000001n);
                vm.call("_object_set");
                vm.lea(VReg.V0, tmpSlot);
                vm.load(VReg.A0, VReg.V0, 0);
                this.emitBoxedStringKey(unscNames[ui], VReg.A1);
                vm.movImm(VReg.A2, 7); // {W,E,C}=true (CreateDataProperty)
                vm.call("_object_set_prop_attr");
            }
            // 字符串键 "Symbol.unscopables"
            vm.lea(VReg.V0, tmpSlot);
            vm.load(VReg.RET, VReg.V0, 0);
            this._reSetProtoProp(protoSlot, "Symbol.unscopables", ACCESSOR_PROP_ATTR); // {w:f,e:f,c:t}
            // well-known Symbol 键
            vm.lea(VReg.A0, "_symwk_unscopables");
            vm.lea(VReg.A1, this.asm.addString("Symbol.unscopables"));
            vm.movImm64(VReg.V1, 0x7ffc000000000000n);
            vm.or(VReg.A1, VReg.A1, VReg.V1);
            vm.call("_symbol_wellknown");
            vm.mov(VReg.S0, VReg.RET);
        ...
        // Map.prototype[@@iterator] = entries；Set.prototype[@@iterator] = values。
        // 两者都必须与对应命名方法保持同一函数对象。双键协议同 Array。
        if ((cfg.name === "Map" || cfg.name === "Set") && typeof cfg.speciesTmpSlot === "string") {
            const tmpSlot = cfg.speciesTmpSlot;
            this._reEnsureSlot(tmpSlot);
            vm.lea(VReg.V0, protoSlot);
            vm.load(VReg.A0, VReg.V0, 0);
            this.emitBoxedStringKey(cfg.name === "Set" ? "values" : "entries", VReg.A1);
            vm.call("_object_get");
            vm.lea(VReg.V1, tmpSlot);
        ...
        // The intrinsic constructor is also an own property of globalThis.
        // Publish it only during first materialisation so later user writes to
        // globalThis.Promise remain observable.
        if (cfg.name === "Promise") {
            vm.lea(VReg.V0, ctorSlot);
            vm.load(VReg.A2, VReg.V0, 0);
            vm.lea(VReg.V0, "_global_this");
            vm.load(VReg.A0, VReg.V0, 0);
            this.emitBoxedStringKey("Promise", VReg.A1);
            vm.call("_object_set");
            vm.lea(VReg.V0, "_global_this");
            vm.load(VReg.A0, VReg.V0, 0);
            this.emitBoxedStringKey("Promise", VReg.A1);
    ...
    emitArrayCtorObject() {
        ...
        // 先执行的 FE 读槽仍 0 → `Object.defineProperty(Array.prototype,…)` 报
        // "target is not an object"(15.2.3.6-4-192 assert.throws 回调)。
        this.emitCollectionCtorObject({
        ...
        // Distinguish a fully populated intrinsic Array.prototype from the empty
        // shell that _ensure_array_proto may create for getPrototypeOf / hole Get.
        // ctorSlot alone is not a sufficient state bit during lazy materialisation.
    ...
    // [底层A] `Array.prototype` 值读:构造器已物化则直接用,否则整体物化。
    // 须与 emitCollectionCtorObject 同判 ctorSlot(非 protoSlot):_ensure_array_proto
    // 会先种空 TYPE_OBJECT 进 proto 槽,W-29 若只看 proto≠0 会跳过 values/@@iterator
    // 落位 → arr[Symbol.iterator] 恒 undefined(test262 makeIterable 等)。
    emitArrayProtoObject() {
        const vm = this.vm;
        const ctorSlot = "_nsobj_array";
    ...
    emitObjectCtorObject() {
        ...
        // Replace process-init's lightweight global placeholder with the fully
        // materialised singleton exactly once.  Later writes to globalThis.Object
        // remain observable because the ready fast path skips this block.
        vm.lea(VReg.V0, ctorSlot);
        vm.load(VReg.A2, VReg.V0, 0);
        vm.lea(VReg.V0, "_global_this");
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("Object", VReg.A1);
        vm.call("_object_set");
        vm.lea(VReg.V0, "_global_this");
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("Object", VReg.A1);
    ...
    emitSetCtorObject() {
        this.emitCollectionCtorObject({
            ...
            methodMemoPrefix: "spm_",
        });
    },
    // WeakSet/WeakMap 一等值:与 Set/Map 同形,独立 ctor/proto 槽(heritage /
    // instanceof / typeof 反射)。方法表暂空——品牌守卫尚拒 weakness;subclass-builtins
    // 仅需构造器身份与 .prototype。new WeakSet 仍走 compileNewExpression 快路。
    emitWeakSetCtorObject() {
        this.emitCollectionCtorObject({
            name: "WeakSet", length: 0, ctorFn: "_weakset_ctor_call",
            ctorSlot: "_nsobj_weakset", protoSlot: "_nsobj_weakset_proto",
            methods: [], sizeGetter: null, statics: [],
            speciesTmpSlot: "_nsobj_weakset_tmp",
        });
    },
    emitWeakMapCtorObject() {
        this.emitCollectionCtorObject({
            name: "WeakMap", length: 0, ctorFn: "_weakmap_ctor_call",
            ctorSlot: "_nsobj_weakmap", protoSlot: "_nsobj_weakmap_proto",
            methods: [], sizeGetter: null, statics: [],
    ...
    emitPromiseCtorObject() {
        this.emitCollectionCtorObject({
            ...
            speciesTmpSlot: "_nsobj_promise_tmp",
        });
    },
    // Proxy 一等值:无 .prototype(规范 26.2.2)。length=2 / name="Proxy" attr 4;
    // 静态 revocable;挂回 globalThis(this.Proxy own, attr 5)。
    emitProxyCtorObject() {
        const vm = this.vm;
        const ctorSlot = "_nsobj_proxy";
        this._reEnsureSlot(ctorSlot);
        const doneL = this.ctx.newLabel("nsproxy_done");
        vm.lea(VReg.V0, ctorSlot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.cmpImm(VReg.RET, 0);
        vm.jne(doneL);
        ...
        vm.movImm(VReg.V1, 0xc105);
        vm.store(VReg.S0, 0, VReg.V1);
        vm.lea(VReg.V1, "_proxy_ctor_call");
        vm.store(VReg.S0, 8, VReg.V1);
        vm.mov(VReg.A0, VReg.S0);
        vm.call("_js_box_function");
        vm.lea(VReg.V1, ctorSlot);
        ...
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("name", VReg.A1);
        vm.lea(VReg.A2, this.asm.addString("Proxy"));
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A2, VReg.A2, VReg.V1);
        vm.call("_closure_prop_set");
        vm.lea(VReg.V0, ctorSlot);
        ...
        vm.movImm(VReg.A2, 4);
        vm.call("_closure_prop_set_attr");
        this.emitMemoizedBuiltinRef("proxy_revocable", "_proxy_revocable", "revocable");
        const stash = this.ctx.allocLocal("__proxy_rev_" + this.nextLabelId());
        vm.store(VReg.FP, stash, VReg.RET);
        vm.lea(VReg.V0, ctorSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("revocable", VReg.A1);
        vm.load(VReg.A2, VReg.FP, stash);
        vm.call("_closure_prop_set");
        vm.lea(VReg.V0, ctorSlot);
        vm.load(VReg.A0, VReg.V0, 0);
        ...
        vm.lea(VReg.V0, "_global_this");
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("Proxy", VReg.A1);
        vm.call("_object_set");
        vm.lea(VReg.V0, "_global_this");
        vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("Proxy", VReg.A1);
    ...
    emitCollectionProtoObject(name) {
        ...
        const protoSlot = "_nsobj_" + key + "_proto";
        const ctorSlot = "_nsobj_" + key;
        this._reEnsureSlot(protoSlot);
        this._reEnsureSlot(ctorSlot);
        const doneL = this.ctx.newLabel("nscollproto_done");
        const emitL = this.ctx.newLabel("nscollproto_emit");
        vm.lea(VReg.V0, protoSlot);
        vm.load(VReg.RET, VReg.V0, 0);
        vm.cmpImm(VReg.RET, 0);
        vm.jeq(emitL);
        // proto 已填且 ctor 已填 → 完整物化,直接返回。仅 proto(gPO 空 ensure)
        // 须补挂方法并复用同一对象 → getPrototypeOf(x) === X.prototype。
        // x64 V0≡RET:ctor 槽检查必须用 V1。旧 lea/load V0 把 RET 换成 ctor,
        // 第二次 `Set.prototype` 返回 Set 函数本身 → P===Set.prototype 恒假,
        // `Set.prototype.add=` 写到构造器,fill/gPO 仍看 _nsobj_set_proto。
        vm.lea(VReg.V1, ctorSlot);
        vm.load(VReg.V1, VReg.V1, 0);
        vm.cmpImm(VReg.V1, 0);
        vm.jne(doneL);
        ...
        else if (name === "WeakMap") this.emitWeakMapCtorObject();
        else if (name === "WeakSet") this.emitWeakSetCtorObject();
    ...
    emitErrorCtorRef(name) {
        ...
        const protoStash = this.ctx.allocLocal(`__errproto_${name}_${this.nextLabelId()}`);
        ...
        // [gOPD 补全] 预建 .prototype(镜像 _closure_prop_get 的 W-41 惰性形态:
        // 新对象 + constructor 回链 attr 5),并落 attr 0——规范错误构造器的
        // prototype 键 {writable:false, enumerable:false, configurable:false}
        // (15.2.3.3-4-216/220)。不预建则 gOPD 侧表 miss → undefined;W-41 惰性建
        // 走 _closure_prop_set 默认 attr 7 → 全真误报。后续 `Error.prototype` 值读
        // 侧表命中同一对象(W-41 分支只在 miss 时触发),身份稳定。
        this.vm.call("_object_new");
        this.vm.call("_box_obj_r");
        this.vm.mov(VReg.S0, VReg.RET);            // S0 = 装箱 proto(跨 call 存活)
        this.vm.mov(VReg.A0, VReg.S0);
        this.emitBoxedStringKey("constructor", VReg.A1);
        this.vm.lea(VReg.V0, label);
        this.vm.load(VReg.A2, VReg.V0, 0);         // A2 = memoized 构造器闭包
        this.vm.call("_object_set");
        this.vm.mov(VReg.A0, VReg.S0);
        this.emitBoxedStringKey("constructor", VReg.A1);
        this.vm.movImm(VReg.A2, BUILTIN_PROP_ATTR); // {w:true,e:false,c:true}
        this.vm.call("_object_set_prop_attr");
        this.vm.store(VReg.FP, protoStash, VReg.S0);
        if (name === "Error") {
            // Error.prototype.toString is an own built-in data property. The
            // generic method trampoline passes its receiver to _error_to_str.
            this.emitMemoizedBuiltinMethodRef("errorproto_toString", "_error_to_str", "toString", 0);
            this.vm.mov(VReg.A2, VReg.RET);
            this.vm.load(VReg.A0, VReg.FP, protoStash);
            this.emitBoxedStringKey("toString", VReg.A1);
            this.vm.call("_object_set");
            this.vm.load(VReg.A0, VReg.FP, protoStash);
            this.emitBoxedStringKey("toString", VReg.A1);
        ...
        this.vm.load(VReg.S0, VReg.FP, protoStash);
        this.vm.lea(VReg.V0, label);
        ...
        this.vm.load(VReg.A0, VReg.V0, 0);
        this.emitBoxedStringKey("prototype", VReg.A1);
        this.vm.movImm(VReg.A2, BUILTIN_CONST_ATTR); // 规范全 false
        this.vm.call("_closure_prop_set_attr");
        this.vm.lea(VReg.V0, label);
        this.vm.load(VReg.RET, VReg.V0, 0);
        this.vm.label(doneL);
    ...
    // %TypedArray%[@@species] getter(规范返 this,length=0)。勿在 runtime _ta_intrinsic
    // 内联安装:会扰动 _engine_symaddr→_aref_static_tramp,eval/class 片段闭包 fnptr 错绑。
    emitEnsureTypedArraySpecies() {
        const vm = this.vm;
        const flag = "_ta_species_installed";
        const tmpSlot = "_ta_species_getter";
        this._reEnsureSlot(flag);
        this._reEnsureSlot(tmpSlot);
        const doneL = this.ctx.newLabel("ta_sp_inst_done");
        vm.lea(VReg.V0, flag);
        vm.load(VReg.V0, 0);
        vm.cmpImm(VReg.V0, 0);
        vm.jne(doneL);
        vm.call("_ta_intrinsic");
        vm.mov(VReg.S0, VReg.RET);             // 装箱 %TypedArray%
        this.emitBuiltinFnClosure("_get_this");
        vm.lea(VReg.V1, tmpSlot);
        vm.store(VReg.V1, 0, VReg.RET);
        vm.lea(VReg.V0, tmpSlot);
    ...
    emitCtorClosureRef(slotKey, typeTag) {
        if (this.engineNoIC) {
            // Reuse the host singleton; see runtime _ta_dynamic_ctor_ref.
            this.vm.movImm(VReg.A0, typeTag);
            this.vm.call("_ta_dynamic_ctor_ref");
            return;
        }
        ...
        this.vm.lea(VReg.V1, label);
        this.vm.store(VReg.V1, 0, VReg.RET);
        // globalThis.X === X:与 Array 同形。`globalThis.Float64Array` 依赖此。
        this.vm.mov(VReg.S0, VReg.RET);
        const skipGlobal = this.ctx.newLabel("taref_skip_gthis");
        this.vm.lea(VReg.V0, "_global_this");
        this.vm.load(VReg.A0, VReg.V0, 0);
        this.vm.cmpImm(VReg.A0, 0);
        this.vm.jeq(skipGlobal); // globalThis 尚未物化则跳过(罕见)
        this.emitBoxedStringKey(slotKey, VReg.A1);
        this.vm.mov(VReg.A2, VReg.S0);
        this.vm.call("_object_set");
        this.vm.lea(VReg.V0, "_global_this");
        this.vm.load(VReg.A0, VReg.V0, 0);
        ...
        this.vm.call("_object_set_prop_attr");
        this.vm.label(skipGlobal);
        // proto.constructor = 本构造器单例(species/result.constructor === TA)
        this.vm.movImm(VReg.A0, typeTag);
        this.vm.mov(VReg.A1, VReg.S0);
        this.vm.call("_ta_link_ctor");
        // ctor.BYTES_PER_ELEMENT(变量路径;静态 X.BPE 仍由访问点内联)。
        if (Object.prototype.hasOwnProperty.call(TA_CTOR_BPE, slotKey)) {
            this.vm.mov(VReg.A0, VReg.S0);
            this.emitBoxedStringKey("BYTES_PER_ELEMENT", VReg.A1);
            this.vm.movImm(VReg.A2, TA_CTOR_BPE[slotKey]);
            this.vm.scvtf(0, VReg.A2);
            this.vm.fmovToInt(VReg.A2, 0);
            this.vm.call("_closure_prop_set");
            this.vm.mov(VReg.A0, VReg.S0);
            this.emitBoxedStringKey("BYTES_PER_ELEMENT", VReg.A1);
            this.vm.movImm(VReg.A2, 0); // w:0 e:0 c:0
            this.vm.call("_closure_prop_set_attr");
        }
    ...
    emitBuiltinMethodRefClosure(helperLabel) {
        // ES 内建方法均为严格函数:裸 `Array.prototype.concat()` 的 this=undefined。
        // 所有方法值闭包 code_ptr 共享 _aref_generic → 侧表登记一次即可。
        if (!this._arefGenericStrictMeta) {
            this._arefGenericStrictMeta = true;
            if (!this._funcMeta) this._funcMeta = [];
            this._funcMeta.push({ label: "_aref_generic", kind: 0x100, name: "", arity: 0 });
        }
        this.vm.movImm(VReg.A0, 24);
        this.vm.call("_alloc");
        this.vm.mov(VReg.S0, VReg.RET);
    ...
    emitBuiltinMethodRefClosureMeta(helperLabel, methodName, arity) {
        ...
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A2, VReg.A2, VReg.V1);
        // define:元数据 arity 回落为 0 时 _closure_prop_set 会把 length 当「已存在不可写」忽略
        vm.call("_closure_prop_define");
        vm.mov(VReg.A0, VReg.S0);
        this.emitBoxedStringKey("length", VReg.A1);
        vm.movImm(VReg.A2, arity);
    ...
    // [gOPD 补全] memoized 方法值闭包:惰性全局槽 _builtinref_<slotKey>(GC 根)缓存
    // emitBuiltinMethodRefClosureMeta 产的 24B 蹦床闭包 → 跨站点/跨次读身份稳定。
    // Array.prototype 方法值读分支与 emitArrayCtorObject 物化 own prop 共用同槽 →
    // `gOPD(Array.prototype,"m").value === Array.prototype.m` 恒等,且与站点执行
    // 顺序无关(每站点自带建槽守卫,不依赖 emitArrayCtorObject once-flag 的
    // 「首个编译点先执行」假设)。RET = 装箱闭包。
    emitMemoizedBuiltinMethodRef(slotKey, helperLabel, methodName, arity) {
        const label = "_builtinref_" + slotKey;
        if (!this._addedBuiltinRefLabels) this._addedBuiltinRefLabels = new Set();
        if (!this._addedBuiltinRefLabels.has(label)) {
            this.asm.addDataLabel(label);
        ...
        const doneL = this.ctx.newLabel("bmref_done");
        this.vm.lea(VReg.V0, label);
        this.vm.load(VReg.RET, VReg.V0, 0);
        this.vm.cmpImm(VReg.RET, 0);
        this.vm.jne(doneL);
        this.emitBuiltinMethodRefClosureMeta(helperLabel, methodName, arity); // RET = 闭包
        this.vm.lea(VReg.V1, label);
        this.vm.store(VReg.V1, 0, VReg.RET);
        this.vm.label(doneL);
    },

    // Array.prototype 具名方法表命中(供 method call 在未知接收者上惰性物化原型)。
    isArrayProtoMethodName(name) {
        return typeof name === "string" &&
            Object.prototype.hasOwnProperty.call(ARRAY_PROTO_METHODS_REF, name);
    },

    // Box S1 (raw super base / parent proto) into A0 as 0x7FFD.
    // Same bytes as functions.js super.method() boxing. V1 scratch (x64 V0≡RET).
    emitBoxSuperBaseAsGetReceiver() {
    ...
    // Object-literal / base-class method: HomeObject ≈ this when called on the
    // defining object. Super base = [[GetPrototypeOf]](this). RET = proto.
    // TypeError if proto is null (MakeSuperPropertyReference / ToObject).
    emitSuperLoadThisProto() {
        const thisOffset = this.ctx.getLocal("__this");
        this.emitGuardDerivedThis();
        if (thisOffset) {
            this.vm.load(VReg.A0, VReg.FP, thisOffset);
        } else {
            this.vm.lea(VReg.V1, "_global_this");
            this.vm.load(VReg.RET, VReg.V1, 0);
            this.vm.call("_box_obj_r");
            this.vm.mov(VReg.A0, VReg.RET);
        }
        this.vm.call("_object_getPrototypeOf");
        const protoOk = this.ctx.newLabel("supth_proto_ok");
        this.vm.shrImm(VReg.V1, VReg.RET, 48);
        this.vm.cmpImm(VReg.V1, 0x7FFA); // null
        this.vm.jne(protoOk);
        // Same _throw_type_error sequence as class super.prop (no new TypeError
        // locals in the method — that path was a second-super SIGSEGV source).
        this.vm.lea(VReg.A0, this.asm.addString("Cannot convert undefined or null to object"));
        this.vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        this.vm.or(VReg.A0, VReg.A0, VReg.V1);
    ...
    // super.prop GET without class superClass: Get(GetPrototypeOf(this), prop)
    // then _maybe_getter with this (data props returned as-is).
    // x64: emitBoxedStringKey → _tag_key_a1 clobbers A0/RET; proto lives in a frame slot.
    emitSuperPropGetFromThis(propName) {
        const thisOffset = this.ctx.getLocal("__this");
        this.emitSuperLoadThisProto();
        const protoOff = this.ctx.allocLocal(`__supth_proto_${this.nextLabelId()}`);
    ...
    // super.prop SET: Set(superBase, key, value, Receiver=this).
    // superBase = GetPrototypeOf(this) (object-literal / no classinfo) or parent
    // prototype (class). Lookup on the base so the own setter is not re-entered
    // (object.x=1 → setter → super.x=v must call proto's setter with this).
    // PutValue ToObject(base) is AFTER RHS (null proto: RHS still runs).
    // Data fallback strictness follows the method (class=strict, sloppy object
    // methods stay sloppy). x64: proto/val in frame slots; key emit clobbers
    // A0/RET. Do not new TypeError here (extra locals = 2nd-super SIGSEGV).
    emitSuperPropSet(propName, rightExpr, computedProp) {
        const thisOffset = this.ctx.getLocal("__this");
        const id = this.nextLabelId();
        const protoOff = this.ctx.allocLocal(`__supset_proto_${id}`);
        const valOff = this.ctx.allocLocal(`__supset_val_${id}`);
        const keyOff = (propName == null) ? this.ctx.allocLocal(`__supset_key_${id}`) : null;
        const strictSet = (this.ctx && this.ctx.inStrictFunction) ||
            (this._currentModuleAst && this._currentModuleAst._bsStrict) ||
            !!(this.ctx && this.ctx.inClass); // class body is always strict

        // GetThisBinding first (uninit this → ReferenceError before RHS).
        this.emitGuardDerivedThis();

        // Spec: evaluate computed key Expression (GetValue) before RHS;
        // ToPropertyKey is PutValue, after both sides and ToObject(base).
        if (propName == null && computedProp) {
            this.compileExpression(computedProp);
            this.vm.store(VReg.FP, keyOff, VReg.RET);
        }

        this.compileExpression(rightExpr);
        this.vm.store(VReg.FP, valOff, VReg.RET);

        // ToObject(superBase) after RHS: null proto TypeError keeps RHS effects.
        if (!this.ctx.superClass) {
            this.emitSuperLoadThisProto();
            this.vm.store(VReg.FP, protoOff, VReg.RET);
        } else {
            this.emitLoadSuperClassInfo(VReg.S1);
            const supInfoOk = this.ctx.newLabel("supset_info_ok");
            const supInfoBad = this.ctx.newLabel("supset_info_bad");
            this.vm.cmpImm(VReg.S1, 0);
            this.vm.jeq(supInfoBad);
            this.vm.cmpImm(VReg.S1, 1);
            this.vm.jne(supInfoOk);
            this.vm.label(supInfoBad);
            this.vm.lea(VReg.A0, this.asm.addString("Cannot convert undefined or null to object"));
            this.vm.movImm64(VReg.V1, 0x7ffc000000000000n);
            this.vm.or(VReg.A0, VReg.A0, VReg.V1);
            this.vm.call("_throw_type_error");
            ...
            this.emitBoxSuperBaseAsGetReceiver();
            this.vm.store(VReg.FP, protoOff, VReg.A0);
        }

        // PutValue: ToPropertyKey after ToObject(base). Object key toString
        // is after RHS (target-super-computed-reference 2nd case).
        if (propName == null && keyOff != null) {
            this.vm.load(VReg.A0, VReg.FP, keyOff);
            this.vm.call("_js_prop_key");
            this.vm.store(VReg.FP, keyOff, VReg.RET);
        }

        if (propName != null) this.emitBoxedStringKey(propName, VReg.A1);
        else this.vm.load(VReg.A1, VReg.FP, keyOff);
        this.vm.load(VReg.A0, VReg.FP, protoOff);
        this.vm.call("_object_get");
        this.vm.mov(VReg.A0, VReg.RET);
        if (thisOffset) this.vm.load(VReg.A1, VReg.FP, thisOffset);
        ...
        this.vm.call("_maybe_setter");
        const didSet = this.ctx.newLabel("supset_did");
        this.vm.cmpImm(VReg.RET, 0);
        this.vm.jne(didSet);
        // Data / miss: Set on receiver (this), not the super base.
        if (thisOffset) this.vm.load(VReg.A0, VReg.FP, thisOffset);
        else this.vm.movImm(VReg.A0, 0);
        if (propName != null) this.emitBoxedStringKey(propName, VReg.A1);
        else this.vm.load(VReg.A1, VReg.FP, keyOff);
        this.vm.load(VReg.A2, VReg.FP, valOff);
        this.vm.call(strictSet ? "_object_set_strict" : "_object_set");
        this.vm.label(didSet);
        this.vm.load(VReg.RET, VReg.FP, valOff);
    },

    // 派生构造器:super() 前读/写 this 须抛 ReferenceError(this-access-restriction 族)。
    // 字段初始化在 super() 语句后才发射,不会误伤 `b = this.a`。
    emitGuardDerivedThis() {
        if (this.ctx.superCalledOff == null) return;
        const ok = this.ctx.newLabel("this_init_ok");
    ...
    // __super_called 可能是裸 0/1,或箭头 lexical super 共享的 box。
    emitLoadSuperCalled(dst) {
        const vm = this.vm;
        if (this.ctx.superCalledOff == null) {
            vm.movImm(dst, 0);
            return;
        }
        if (this.ctx.boxedVars && this.ctx.boxedVars.has("__super_called")) {
            vm.load(dst, VReg.FP, this.ctx.superCalledOff);
            vm.load(dst, dst, 0);
            return;
        }
        vm.load(dst, VReg.FP, this.ctx.superCalledOff);
    ...
    emitMarkSuperCalled() {
        if (this.ctx.superCalledOff == null) return;
        const vm = this.vm;
        // SuperCall returns BindThisValue(result). x64 V0≡RET: movImm(V0,1) made
        // `value = super()` the denormal 5e-324 (bit pattern 1). Save first.
        const retOff = this.ctx.allocLocal(`__supret_${this.nextLabelId()}`);
        vm.store(VReg.FP, retOff, VReg.RET);
        // BindThisValue: already initialized → ReferenceError (second super()).
        // Check before the flag write. Construct already ran (spec order).
        const onceOk = this.ctx.newLabel("super_once_ok");
        this.emitLoadSuperCalled(VReg.V1);
        vm.cmpImm(VReg.V1, 0);
        vm.jeq(onceOk);
        vm.lea(VReg.A0, this.asm.addString("Super constructor may only be called once"));
        vm.movImm64(VReg.V1, 0x0000ffffffffffffn);
        vm.and(VReg.A0, VReg.A0, VReg.V1);
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A0, VReg.A0, VReg.V1);
        vm.call("_throw_reference_error");
        vm.label(onceOk);
        if (this.ctx.boxedVars && this.ctx.boxedVars.has("__super_called")) {
            vm.load(VReg.V1, VReg.FP, this.ctx.superCalledOff);
            vm.movImm(VReg.V2, 1);
            vm.store(VReg.V1, 0, VReg.V2);
        } else {
            vm.movImm(VReg.V2, 1);
            vm.store(VReg.FP, this.ctx.superCalledOff, VReg.V2);
        }
        // Derived instance fields run AFTER super() (ES InitializeInstanceElements).
        // Hook here so any super() shape (not only a top-level ExpressionStatement)
    ...
    // _object_has);命中则 RET = [[Get]](obj, name)(须解 accessor——成员读走
    // _object_get_ic;此处对标 push + _object_get + _maybe_getter),否则回退普通词法解析。
    // Object Environment Record HasBinding(N):HasProperty 后若 @@unscopables
    // 为 Object 且 ToBoolean(Get(unscopables,N)) 则视作未绑定(跳 missL)。
    // binding 在 FP+fpSlot;命中落穿、blocked/无属性→missL。
    // HasProperty(bindingObject, N):须走 `in`/`_prop_in`(原型链 + Proxy has),
    // 不能用 own-only 的 `_object_has`(Proxy 上恒 false,has 陷阱不触发)。
    _emitObjectEnvHasProperty(fpSlot, name, missL) {
        this.emitBoxedStringKey(name, VReg.A0);
        this.vm.call("_js_prop_key");
        this.vm.mov(VReg.A0, VReg.RET);
        this.vm.call("_getStrContent");
        const kc = this.ctx.allocLocal(`__with_kc_${this.nextLabelId()}`);
        this.vm.store(VReg.FP, kc, VReg.RET);
        this.vm.load(VReg.A0, VReg.FP, fpSlot);
        this.vm.call("_js_unbox");
        this.vm.mov(VReg.A0, VReg.RET);
        this.vm.load(VReg.A1, VReg.FP, kc);
        this.vm.call("_prop_in"); // 裸 0/1;Proxy has 可抛
        this.vm.cmpImm(VReg.RET, 0);
        this.vm.jeq(missL);
    },

    _emitObjectEnvHasBinding(fpSlot, name, missL) {
        const vm = this.vm;
        const hitL = this.ctx.newLabel("with_has_hit");
        const isObjL = this.ctx.newLabel("with_has_isobj");
        this._emitObjectEnvHasProperty(fpSlot, name, missL);
        // Get(bindings, @@unscopables) — well-known Symbol 键(规范 + getter 测试)
        vm.lea(VReg.A0, "_symwk_unscopables");
        vm.lea(VReg.A1, this.asm.addString("Symbol.unscopables"));
        vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        vm.or(VReg.A1, VReg.A1, VReg.V1);
        ...
        vm.call("_to_boolean");
        vm.cmpImm(VReg.RET, 0);
        vm.jne(missL); // blocked
        vm.label(hitL);
    },

    _hasAnyWithScope() {
        return (this.ctx.withScopes && this.ctx.withScopes.length > 0) ||
            (this.ctx.outerWithScopes && this.ctx.outerWithScopes.length > 0);
    },

    _isOwnBinding(name) {
        return !!(this.ctx.ownBindingNames && this.ctx.ownBindingNames[name]);
    },

    _emitWithIdentifierGet(slot, name, doneL) {
        const missL = this.ctx.newLabel("with_miss");
        const goneL = this.ctx.newLabel("with_gbv_gone");
        this._emitObjectEnvHasBinding(slot, name, missL);
        // GetBindingValue 再做一次 HasProperty(Proxy 测试要第二条 has 日志)。
        // HasBinding 已命中本 Object Env 后 !HasProperty:
        // sloppy → undefined; strict → ReferenceError。不得 miss 到下一环境
        ...
        this.vm.jmp(doneL);
        this.vm.label(goneL);
        const strictGet = (this.ctx && this.ctx.inStrictFunction) ||
            (this._currentModuleAst && this._currentModuleAst._bsStrict);
        if (strictGet) {
    ...
    _compileWithIdentifier(expr) {
        ...
        for (let i = cur.length - 1; i >= 0; i--) {
            this._emitWithIdentifierGet(cur[i], name, doneL);
        }
        // 本函数 varEnv 在外层 with 之内、当前 with 之外。
        if (this._isOwnBinding(name)) {
            ...
            for (let i = outer.length - 1; i >= 0; i--) {
                this._emitWithIdentifierGet(outer[i], name, doneL);
            }
        }
        this._inWithResolve = true;
        this.compileIdentifier(expr);
    ...
    isUnresolvableIdentifier(expr) {
        ...
        // for-in/of 头部 RHS 的 BoundNames 是已解析的 TDZ 绑定,不是 unresolvable。
        // 若此处返 true,typeof 会走"未解析名"快路返回 "undefined" 而不抛。
        if (expr._tdzHeadRhs) return false;
        const name = expr.name;
        // with(obj) 作用域内标识符要先查 with 对象属性,静态判不了 → 保守当可解析
        if (this._hasAnyWithScope && this._hasAnyWithScope()) return false;
        if (IDENT_BUILTIN_NAME_SET[name]) return false;
        if (IDENT_KNOWN_GLOBAL_NAME_SET[name]) return false;
        if (ERR_CTOR_NAME_SET[name]) return false;
    ...
    compileIdentifier(expr) {
        ...
        // for-in/of 头部 RHS 里对 BoundNames 的读(含其中创建的闭包)恒 TDZ:
        // 该环境求值完即丢弃,永不初始化,不能与循环体绑定共用 box。
        if (expr._tdzHeadRhs) {
        ...
        if (!this._inWithResolve && this._hasAnyWithScope()) {
            this._compileWithIdentifier(expr);
            return;
        }

        // 热路径：先查局部。自编译绝大多数标识符是局部/形参，
        // 避免先走数十次全局名字符串比较；局部遮蔽全局名亦更符合词法。
        const offset = this.ctx.getLocal(name);
        if (offset) {
            // [L2-③ TDZ] 默认参数自引用(x=x)/后向引用(x=y,y=1):
            // 形参名在默认值评估期记入 tdzParams,此处以 ReferenceError 守卫
            if (this.ctx.tdzParams && this.ctx.tdzParams.has(name)) {
                this.emitThrowReferenceError(name + " is not defined");
                return;
            }
            ...
            if (isBoxed) {
                ...
                // 值级哨兵守卫:仅 let/const/import 或 _tdz 标点需要(var/function/class/
                // 形参永不 TDZ)。此前对一切装箱读无条件发射 → 自编译数万次空守卫。
                // [m120] 同函数体声明点已写过后(_tdzClearedLocals)在 _bindingMayBeTdz 内跳过。
                if (expr._tdz || this._bindingMayBeTdz(name)) {
        ...
        // Synthetic shim imports (notably `__eval`) are initialized into a
        // module-main local slot.  Nested user functions intentionally do not
        // inherit that slot, but they still carry the import binding in their
        // module context.  Resolve the binding through the module export here
        // before the generic function-alias path; otherwise each nested
        // reference allocates its own closure and breaks intrinsic identity
        // (`eval === eval`, and callback thisArg identity).
        if (name === "__eval" && this.getImportBindingForLocal && this._currentModuleAst) {
            const _evalImport = this.getImportBindingForLocal(this._currentModuleAst, name);
            if (_evalImport && !_evalImport.isNamespace &&
                typeof _evalImport.sourceModuleIndex === "number") {
                this.vm.movImm(VReg.A0, _evalImport.sourceModuleIndex);
                this.vm.lea(VReg.A1, this.asm.addString(_evalImport.importedName || name));
        ...
        if (name === "Object" && !this.objectNameShadowed()) {
            this.emitObjectCtorObject();
            // Object is a writable global binding.  Materialisation installs
            // the intrinsic as globalThis.Object, but subsequent writes through
            // the global object must be observed by bare identifier reads.
            this.vm.lea(VReg.V0, "_global_this");
            this.vm.load(VReg.A0, VReg.V0, 0);
            this.emitBoxedStringKey("Object", VReg.A1);
            this.vm.call("_object_get");
            this.vm.mov(VReg.A0, VReg.RET);
            this.vm.lea(VReg.V0, "_global_this");
            this.vm.load(VReg.A1, VReg.V0, 0);
            this.vm.call("_maybe_getter");
            return;
        }
        // [gOPD 补全] 裸 Function 一等化:惰性物化单例真闭包(替换旧哨兵 3)。
        // `new Function(...)` 在 NewExpression 里已单独改派 __makeFunction,不经此;
        // `x instanceof Function` 的 RHS 经此得单例,jsvalue.js _iof_chk_fn 以
        // _fnctor_singleton 指针相等继续走原 Function 分支(快路语义不变)。
        if (name === "Function" && !(this.ctx.getLocal && this.ctx.getLocal("Function")) &&
            !(this.ctx.getFunction && this.ctx.getFunction("Function"))) {
            // The runtime may already have materialised the same singleton
            // while resolving %AsyncFunction%/%GeneratorFunction% inheritance.
            // Use that complete runtime constructor rather than returning early
            // from the compiler-side fill path with split identity.
            this.vm.call("_ensure_function_ctor_runtime");
            return;
        }
        ...
        if (name === "WeakSet" && !this.collectionNameShadowed("WeakSet")) {
            this.emitWeakSetCtorObject();
            return;
        }
        if (name === "WeakMap" && !this.collectionNameShadowed("WeakMap")) {
            this.emitWeakMapCtorObject();
            return;
        }
        ...
        if (name === "Proxy" && !this.collectionNameShadowed("Proxy")) {
        ...
        // [BigInt 一等值] BigInt 构造函数 + prototype（ToObject(0n) instanceof BigInt）
        if (name === "BigInt" &&
            !(this.ctx.getLocal && this.ctx.getLocal("BigInt")) &&
            !(this.ctx.getFunction && this.ctx.getFunction("BigInt"))) {
            this.emitBigIntCtorObject();
            return;
        }
        // [Error 构造器一等值] 裸 TypeError/RangeError/... 作值 → memoized 闭包(.name 就绪、
        // 可传递、`===` 稳定、typeof "function")。用户局部/函数遮蔽同名时退回词法解析。
        if (ERR_CTOR_NAME_SET[name] &&
        ...
        // [Reflect 一等值] 裸 `Reflect`(反射位):惰性物化真命名空间对象 →
        // typeof "object"。Reflect.get(...) 等调用快路先于本路径命中。
        if (name === "Reflect" && !this.reflectNameShadowed()) {
            this.emitReflectNamespaceObject();
            return;
        }
        ...
        // 误命中 → 被错编成伪构造器闭包(打印 [Function])。合法值恒为 number。
        if (typeof TA_CTOR_TAGS[name] === "number" &&
            !(this.ctx.getLocal && this.ctx.getLocal(name)) &&
            !(this.ctx.getFunction && this.ctx.getFunction(name))) {
        ...
        // [L2-③ TDZ·AST 标点] 无局部槽时仍可能带 _tdzRefName（防御）
        if (expr._tdzRefName) {
            const tdzMsg = this.asm.addString(name + " is not defined");
            this.vm.lea(VReg.A0, tdzMsg);
            this.vm.movImm64(VReg.V1, 0x7ffc000000000000n);
        ...
        if (globalLabel) {
                ...
                if (funcLabel) {
                    ...
                    this.vm.call("_alloc");
                    this.vm.mov(VReg.S0, VReg.RET);
                    // 声明作值与表达式同构:一律 CLOSURE_MAGIC。async 声明的标签处是
                    // emitAsyncMethodStub,compileClosureCall/compileMethodCall 的普通
                    // 闭包路径都会调到 stub(自建协程+Promise)。不再写 0xa51c——
                    // compileMethodCall 旧只认 0xc105,0xa51c 会被当裸代码指针
                    // callIndirect → SIGBUS(foo.call / .apply)。
                    ...
                    this.vm.store(VReg.V1, 0, VReg.RET); // memoize
                    // 顶层 function* 声明作值:与表达式同,gOPD 须见自有 prototype。
                    if (declNode && (declNode.isGenerator === true || declNode.generator === true)) {
                        this.emitFnOwnPrototype(declNode.async === true || declNode.isAsync === true);
                    }
                ...
                this.vm.call("_get_module_export");
            } else if (name === "__detachArrayBuffer") {
                // test262 HOST_SHIMS: $262.detachArrayBuffer → __detachArrayBuffer(buffer)
                this.emitMemoizedBuiltinRef("host_detach_ab", "_arraybuffer_detach", "__detachArrayBuffer");
                return;
            } else if (name === "print") {
                // 内置函数 print - 生成一个包装闭包
                this.vm.movImm(VReg.A0, 16);
                ...
                // [L2-② / L1 global] 编译期未解析的裸标识符:
                // 规范 Global Environment 的 object Environment Record:先 HasProperty(global, name);
                // 有 → [[Get]];无 → ReferenceError。此前一律抛 → `this.p1=1; p1` 恒崩
                ...
                if (this.isUnresolvableIdentifier &&
                    ...
                    const doneL = this.ctx.newLabel("gident_done");
                    this.vm.lea(VReg.V0, "_global_this");
                    this.vm.load(VReg.RET, VReg.V0, 0);
                    this.vm.call("_box_obj_r"); // RET = boxed globalThis (x64 A0≢RET)
                    // FP 槽保 global:不可用 push——嵌套在二元运算已 push 左值时会掏错槽
                    const gOff = this.ctx.allocLocal(`__gident_${this.nextLabelId()}`);
                    ...
                    this.vm.call("_object_get");
                    this.vm.mov(VReg.A0, VReg.RET);
                    this.vm.load(VReg.A1, VReg.FP, gOff); // thisArg = global
                    this.vm.call("_maybe_getter");
                    this.vm.jmp(doneL);
                    this.vm.label(missL);
                    this.emitThrowReferenceError(name + " is not defined");
    ...
    _fnNameLength(objExpr) {
        ...
        if (node.type === "ClassDeclaration" || node.type === "ClassExpression") {
            // static name()/length() (method/get/set/gen/field) overwrite the
            // builtin class name/length. Computed static keys are runtime-unknown
            // (`static [attr]()` with attr==="name"|"length") so they also skip
            // the compile-time fold. (let/const fn-name-class: xCls2.name !== "xCls2")
            if (node.body && Array.isArray(node.body)) {
                for (const m of node.body) {
                    if (!m || !m.static || !m.key) continue;
                    if (m.computed) return null;
                    const kn = (m.key.name != null) ? m.key.name : m.key.value;
                    if (kn === "name" || kn === "length") return null;
                }
            }
    ...
    // `?.` node, so `.c` / `.#f` / call / further members compiled as
    // ordinary access of the already-undefined intermediate (TypeError).
    // Walk Member.object / Call.callee for the innermost optional; if this
    // node is a continuation, evaluate that root once, skip to undefined
    // on nullish, else recompile with the `?.` cleared (so later `?.` still
    // short-circuit independently). Snapshot the root so CallExpression
    // bases are not evaluated twice.
    _findInnermostOptional(node) {
        let found = null;
        let n = node;
        while (n) {
            if ((n.type === "MemberExpression" || n.type === "CallExpression") && n.optional)
                found = n;
    ...
    _emitOptionalChainContinuation(node) {
        const opt = this._findInnermostOptional(node);
        if (!opt || opt === node) return false;
        const isCall = opt.type === "CallExpression";
        const base = isCall ? opt.callee : opt.object;
        const skipLabel = this.ctx.newLabel("optcont_skip");
        const endLabel = this.ctx.newLabel("optcont_end");
        this.compileExpression(base);
        this.emitNullishGuardToLabel(VReg.RET, skipLabel);
        // Identifier/this/simple member: recompile the original chain so
        // `a?.b().c` keeps method this === a. Snapshot only CallExpression
        // roots (`f()?.b.c`) to avoid a second call (super() twice).
        let savedBase = null;
        if (base && base.type === "CallExpression") {
            const tmpName = `__optcont_${this.nextLabelId()}`;
            const tmpOff = this.ctx.allocLocal(tmpName);
            this.vm.store(VReg.FP, tmpOff, VReg.RET);
            const ident = { type: "Identifier", name: tmpName };
            savedBase = base;
            if (isCall) opt.callee = ident;
            else opt.object = ident;
        }
        const savedOpt = opt.optional;
        opt.optional = false;
        if (node.type === "CallExpression") this.compileCallExpression(node);
        else this.compileMemberExpression(node);
        opt.optional = savedOpt;
        if (savedBase) {
            if (isCall) opt.callee = savedBase;
            else opt.object = savedBase;
        }
    ...
    compileMemberExpression(expr) {
        ...
        if (expr.optional) {
            ...
            // 0 的根因)。短路时(skip)object 之后的成员/下标不求值,语义正确。
            // 标识符/简单成员可安全再求值。CallExpression(含 super())再求值会双 SuperCall
            // →「may only be called once」(optional-chaining/call-expression.js)。
            expr.optional = false;
            if (expr.object && expr.object.type === "CallExpression") {
                const tmpName = `__optmem_c_${this.nextLabelId()}`;
                const objOff = this.ctx.allocLocal(tmpName);
                this.vm.store(VReg.FP, objOff, VReg.RET);
                const savedObject = expr.object;
                expr.object = { type: "Identifier", name: tmpName };
        ...
        if (expr.computed && expr.object && expr.object.type === "SuperExpression") {
            ...
            this.emitGuardDerivedThis();
            if (!this.ctx.superClass) {
                // Object-literal super[k]: Get(GetPrototypeOf(this), key).
                // Spec: key Expression GetValue, then GetSuperBase, then
                // ToPropertyKey (key.toString may setPrototypeOf).
                const keyOff = (keyName === null)
                    ? this.ctx.allocLocal(`__supthc_key_${this.nextLabelId()}`) : null;
                if (keyName === null) {
                    this.compileExpression(prop);
                    this.vm.store(VReg.FP, keyOff, VReg.RET);
                }
                this.emitSuperLoadThisProto();
                const protoOff = this.ctx.allocLocal(`__supthc_proto_${this.nextLabelId()}`);
                this.vm.store(VReg.FP, protoOff, VReg.RET);
                if (keyName !== null) {
            ...
            // RegExp is represented by a shim constructor closure rather than
            // a classinfo record.  Materialise/read its prototype directly for
            // `super[Symbol.*]` accesses; loading props_ptr from the zero
            // classinfo sentinel would otherwise yield an undefined method.
            if (this.ctx.superClass === "RegExp" && !this.ctx.superClassExpr &&
                !this.ctx.inStaticMethod && this.emitRegExpProtoObject) {
                this.emitRegExpProtoObject(); // RET = boxed RegExp.prototype
                this.vm.emitMaskLoad(VReg.V1);
                this.vm.andMaskReg(VReg.S1, VReg.RET, VReg.V1);
            } else {
                this.emitLoadSuperClassInfo(VReg.S1);
                // [Guard] 父类无 classinfo(0)或 extends null 哨兵(1)→ TypeError
                const supInfoOk = this.ctx.newLabel("supc_info_ok");
                const supInfoBad = this.ctx.newLabel("supc_info_bad");
                this.vm.cmpImm(VReg.S1, 0);
                this.vm.jeq(supInfoBad);
                this.vm.cmpImm(VReg.S1, 1);
                this.vm.jne(supInfoOk);
            ...
            this.emitBoxSuperBaseAsGetReceiver();
            // Keep the boxed super base across key evaluation.  Compiling a
            // computed key (especially `Symbol.replace`) uses A0/RET itself;
            // passing that clobbered register to _object_get previously made
            // valid inherited symbol methods appear missing.
            const superBaseOff = this.ctx.allocLocal(`__supc_base_${this.nextLabelId()}`);
        ...
        if (expr.computed) {
            ...
            if (computedPropName !== null) {
                ...
                if ((expr.property.type === "Literal" || expr.property.type === "StringLiteral") &&
                    ...
                    if (computedPropName === "length") {
                        this.compileExpression(expr.object);
                        this.vm.mov(VReg.A0, VReg.RET);
                        if (!isInternalStringSourcePath(this.sourcePath)) {
                            this.vm.call("_js_length_public");
                        } else {
                    ...
                    if (this.isCanonicalIndexString(computedPropName)) {
                        ...
                        // External/user strings expose UTF-16 code-unit
                        // indexing.  Keep the byte-oriented helper for
                        // compiler/runtime source modules (their strings are
                        // lexer backing buffers) to preserve self-hosting.
                        this.vm.call(isInternalStringSourcePath(this.sourcePath)
                            ? "_str_index_char" : "_str_utf16_at"); // 越界返 undefined
                        return;
                    }
                    // s["charAt"] 等作**值读取**:String.prototype 物化后从原型取方法
                    // 闭包(字符串 tag 走 IC/_object_get 恒 miss)。调用位经
                    // compileStringMethod,不经此。
                    const _shComp = ArefMethodRef.string[computedPropName];
                    if (Array.isArray(_shComp) && this.emitStringProtoObject) {
                        this.emitStringProtoObject(); // RET = String.prototype
                        this.vm.mov(VReg.A0, VReg.RET);
                ...
                this.compileExpression(expr.object);
                // [W-29] "Symbol.*" 键可能命中 Array.prototype 的双键协议(字符串
                // "Symbol.iterator" 等):数组不存 __proto__ 指针,运行期属性读走全局
                // _nsobj_array_proto 槽——若此前从未物化(数组字面量+首次读该键),
                // 槽为空 → 恒 undefined(`id([1])[Symbol.iterator]` 得 undefined、
                // 调 undefined() 崩「not a function」;yield*/for-await 的 unknown
                // 接收者路径同根)。物化幂等、无副作用,先物化再读。**物化毁 RET(返
                ...
                if (computedPropName.startsWith("Symbol.")) {
                    ...
                    // is hung by emitStringCtorObject — must materialize first.
                ...
                // of the well-known symbol stores the pointer key. String-key
                // miss → retry well-known via _subscript_get (getters run).
                const wkName = computedPropName && computedPropName.startsWith("Symbol.")
                    ? computedPropName.slice(7) : null;
                const wkNames = ["iterator", "asyncIterator", "hasInstance", "isConcatSpreadable",
                    "match", "matchAll", "replace", "search", "species", "split", "toPrimitive",
                    "toStringTag", "unscopables"];
                if (wkName && wkNames.includes(wkName)) {
                    // _subscript_get may clobber A0. Keep the receiver in an FP
                    // slot so the legacy string-key fallback reloads the real base.
                    const wkRecvOff = this.ctx.allocLocal(`__wk_recv_${this.nextLabelId()}`);
                    this.vm.store(VReg.FP, wkRecvOff, VReg.RET);
                    this.vm.lea(VReg.A0, "_symwk_" + wkName);
                    this.vm.lea(VReg.A1, this.asm.addString(computedPropName));
                    this.vm.movImm64(VReg.V1, 0x7ffc000000000000n);
                    this.vm.or(VReg.A1, VReg.A1, VReg.V1);
                    this.vm.call("_symbol_wellknown");
                    this.vm.mov(VReg.V6, VReg.RET); // V6=符号键
                    this.vm.load(VReg.A0, VReg.FP, wkRecvOff);
                    this.vm.mov(VReg.A1, VReg.V6);
                    this.vm.call("_subscript_get");
                    const wkDone = this.ctx.newLabel("symwk_done");
                    const wkStr = this.ctx.newLabel("symwk_strfb");
                    this.vm.movImm64(VReg.V1, 0x7ffb000000000000n);
                    this.vm.cmp(VReg.RET, VReg.V1);
                    this.vm.jeq(wkName === "iterator" || wkName === "asyncIterator" ? wkStr : wkDone);
                    this.vm.jmp(wkDone);
                    // Async/iterator implementations still use the legacy
                    // string-key protocol on generated iterator objects; keep
                    // that compatibility fallback only for those two keys.
                    if (wkName === "iterator" || wkName === "asyncIterator") {
                        this.vm.label(wkStr);
                        this.vm.load(VReg.RET, VReg.FP, wkRecvOff);
                        this.emitObjectGetIC(computedPropName);
                    }
                    this.vm.label(wkDone);
                    return;
                }
            ...
            // RegExpBuiltinExec's numeric entries are strings.  When a test
            // indexes a captured astral string (`match[0][0]`), the outer
            // member's receiver is syntactically a RegExp result element even
            // though ordinary array element inference is intentionally
            // unknown.  Route this narrow shape through the UTF-16 bridge.
            if (!isInternalStringSourcePath(this.sourcePath) &&
                isRegExpResultElement(expr.object, this.ctx) &&
                expr.property && (expr.property.type === "Literal" || expr.property.type === "NumericLiteral") &&
                typeof expr.property.value === "number") {
                this.compileExpression(expr.object);
                this.vm.mov(VReg.A0, VReg.RET);
                this.vm.movImm(VReg.A1, Math.trunc(expr.property.value));
                this.vm.call("_str_utf16_at");
                return;
            }
            ...
            if (objType === "String") {
                if (expr.property.type === "Literal" && typeof expr.property.value === "number") {
                    ...
                    // See the static-string-key path above: public indexing
                    // uses UTF-16 units, while internal source stays byte-wise.
                    this.vm.call(isInternalStringSourcePath(this.sourcePath)
                        ? "_str_index_char" : "_str_utf16_at");
                } else {
                    // 动态 s[k]:原键交给 _subscript_get(内部 key_int + named)。
                    // 仅 _str_index_char 会把 k="length"/方法名/-1 全打成 undefined。
                ...
                if (idxIsInt) {
                    ...
                    this.vm.store(VReg.FP, recvOff, VReg.A0);
                    ...
                    this.vm.jne(slow);
                    // defineProperty/accessor/稀疏元素都在数组侧表；有侧表的数组必须
                    // 交给通用 Get，不能让 dense 槽遮蔽 accessor 或其 abrupt completion。
                    this.vm.loadByte(VReg.V3, VReg.V2, 1);
                    this.vm.andImm(VReg.V3, VReg.V3, 2);        // ARR_HAS_SIDETABLE
                    ...
                    this.vm.jge(slow);
                    this.vm.load(VReg.V3, VReg.V2, 16);          // capacity
                    this.vm.cmp(VReg.V1, VReg.V3);
                    this.vm.jge(slow);                           // sparse logical hole
                    ...
                    // 真 hole(槽 0)不能在快路直接当 undefined：数组元素访问必须
                    // 继续走 Array.prototype 的 Get（包括 accessor/proxy/异常）。
                    this.vm.cmpImm(VReg.RET, 0);
                    this.vm.jeq(slow);
                    this.vm.jmp(done);
                    this.vm.label(slow);
            ...
            // [m119] VReg.X → 串字面量; [m120] Type.X → 串字面量
            if (expr.object && expr.object.type === "Identifier" &&
                expr.object.name === "VReg" &&
                HOT_VREG_KEYS[propName] === 1 &&
                !(this.ctx.getLocal && this.ctx.getLocal("VReg"))) {
                this.compileStringValue(propName);
                return;
            }
            if (expr.object && expr.object.type === "Identifier" &&
                expr.object.name === "Type" &&
                typeof HOT_TYPE_CONST[propName] === "string" &&
                !(this.ctx.getLocal && this.ctx.getLocal("Type"))) {
                this.compileStringValue(HOT_TYPE_CONST[propName]);
                return;
            }

            // Error subclasses carry their own prototype.constructor just like
            // every other class.  The generic property path now also exposes
            // the canonical built-in Error constructors, so the historical
            // name-based remapping (`name === "Error"` -> Error) is both
            // unnecessary and wrong for `class MyError extends Error {}`.
            if (propName === "constructor" && expr.object &&
                ...
                const ctorBrand = this.ctx.newLabel("ctor_brand");
                const ctorFb = this.ctx.newLabel("ctor_fb");
                const ctorRecv = this.ctx.allocLocal(`__ctor_recv_${cid}`);
                const ctorValue = this.ctx.allocLocal(`__ctor_value_${cid}`);
                this.compileExpression(expr.object);
                this.vm.store(VReg.FP, ctorRecv, VReg.RET);
                this.emitObjectGetIC("constructor");
                this.vm.store(VReg.FP, ctorValue, VReg.RET);
                // A real prototype constructor (user subclass or materialized
                // built-in) wins.  Only Object/undefined indicates the legacy
                ...
                this.vm.load(VReg.RET, VReg.FP, ctorValue);
            ...
            if (expr.object && expr.object.type === "SuperExpression") {
                if (!this.ctx.superClass) {
                    this.emitSuperPropGetFromThis(propName);
                    return;
                }
                const thisOffset = this.ctx.getLocal("__this");
                this.emitGuardDerivedThis();
                ...
                const supInfoBad2 = this.ctx.newLabel("supc_info_bad2");
                this.vm.cmpImm(VReg.S1, 0);
                this.vm.jeq(supInfoBad2);
                this.vm.cmpImm(VReg.S1, 1);
                this.vm.jne(supInfoOk2);
                this.vm.label(supInfoBad2);
                this.vm.lea(VReg.A0, this.asm.addString("Cannot convert undefined or null to object"));
                this.vm.movImm64(VReg.V1, 0x7ffc000000000000n);
                this.vm.or(VReg.A0, VReg.A0, VReg.V1);
                this.vm.call("_throw_type_error"); // 不返回
            ...
            // [test262 S1 泛型数组方法] Array.prototype.<m> 作值读取。
            // [gOPD 补全] 改发 **memoized** 方法值闭包(槽 _builtinref_apm_<m>,与
            // emitArrayCtorObject 物化 own prop 同槽)—— 使
            // `Array.prototype.every === Array.prototype.every` 与
            // `gOPD(Array.prototype,"every").value === Array.prototype.every` 恒等成立
            // (此前逐次新建 24B 闭包,gOPD 读物化 own prop → 身份漂移,
            // 15.2.3.3-4-40/54/59 FAIL)。不走「物化原型再 _object_get」:
            // memoize 槽每站点自带建槽守卫,与执行顺序无关(emitArrayCtorObject
            // 亦已改运行时槽守卫,见上)。
            // helper 同源不变(表内 _agen_*/_fpg_arr_push 与物化落位逐字一致),泛型调用
            // 形态 `Array.prototype.map.call(arrayLike, fn)` 语义不变。
            // 仅静态链 Array.prototype 且 Array 未被局部遮蔽时触发;编译器源不含此
            // 模式,自举字节不变。[#32 守卫] 命中判定经编译器自建表逐项 === 比较。
            if (expr.object && expr.object.type === "MemberExpression" && !expr.object.computed &&
                expr.object.object && expr.object.object.type === "Identifier" &&
                ...
                let _apEntry = null;
                for (let i = 0; i < ARRAY_PROTO_METHODS.length; i = i + 1) {
                    if (ARRAY_PROTO_METHODS[i][0] === propName) { _apEntry = ARRAY_PROTO_METHODS[i]; break; }
                }
                if (_apEntry) {
                    this.emitMemoizedBuiltinMethodRef("apm_" + _apEntry[0],
                        _apEntry[1], _apEntry[0], _apEntry[2]);
                    return;
                }
            }

            // Set.prototype.<m> / Map.prototype.<m> 值读:与物化 own prop 同 memo 槽
            // (_builtinref_spm_*/mpm_*)。`var setAdd = Set.prototype.add` 必须是
            // 可调用函数(set-iterable-calls-add 先捕获再覆盖)。
            if (expr.object && expr.object.type === "MemberExpression" && !expr.object.computed &&
                expr.object.object && expr.object.object.type === "Identifier" &&
                expr.object.property && expr.object.property.name === "prototype" &&
                !this.collectionNameShadowed(expr.object.object.name)) {
                const _cn = expr.object.object.name;
                let _cmEntry = null, _cmPrefix = null, _cmTable = null;
                if (_cn === "Set") { _cmTable = SET_PROTO_METHODS; _cmPrefix = "spm_"; }
                else if (_cn === "Map") { _cmTable = MAP_PROTO_METHODS; _cmPrefix = "mpm_"; }
                if (_cmTable) {
                    for (let i = 0; i < _cmTable.length; i = i + 1) {
                        if (_cmTable[i][0] === propName) { _cmEntry = _cmTable[i]; break; }
                    }
                }
                if (_cmEntry) {
                    this.emitMemoizedBuiltinMethodRef(_cmPrefix + _cmEntry[0],
                        _cmEntry[1], _cmEntry[0], _cmEntry[2]);
                    return;
                }
            }
            ...
            // (cab* 分支),不经此路径 → 既有字节不变。bind 取值同槽 fnproto_bind
            // (_fp_bind_tramp 已存在;挂 own 后 4-611 / Function.prototype.bind 一等)。
            ...
            if (expr.object && expr.object.type === "MemberExpression" && !expr.object.computed &&
                ...
                else if (propName === "bind") _fph = "_fp_bind_tramp";
                if (typeof _fph === "string") {
                    // Read through the materialized prototype so the direct
                    // static spelling and reflective gOPD share one identity.
            ...
            if (expr.object && expr.object.type === "Identifier" &&
                ...
                if (typeof _nsHelper === "string") {
                    // Object 静态值读经构造器侧表:物化时已挂同槽 memoized 闭包 →
                    // 未覆盖时与 emitMemoizedBuiltinRef 身份一致;用户
                    // `Object.keys = fake` 后侧表覆盖须被读回(values/tamper-with-object-keys)。
                    // Math/Date/Array 仍直读 memoize(覆盖较少见,保持旧字节)。
                    if (expr.object.name === "Object") {
                        // 侧表优先(用户覆盖);miss/undefined 回落 memoize(与 name/length 同形)。
                        // 避免 Object 未完整物化时 _closure_prop_get 得 undefined 拖垮 harness。
                        const okDone = this.ctx.newLabel("obj_static_done");
                        this.emitObjectCtorObject();
                        this.vm.mov(VReg.S0, VReg.RET);
                        this.vm.mov(VReg.A0, VReg.S0);
                        this.emitBoxedStringKey(propName, VReg.A1);
                        ...
                        this.emitMemoizedBuiltinRef(
                            "object_" + propName, _nsHelper, propName);
                        this.vm.label(okDone);
                        return;
                    }
                    this.emitMemoizedBuiltinRef(
            ...
            if (expr.object.type === "Identifier" && expr.object.name === "Symbol" &&
                ...
                // Reading any well-known symbol also makes the %Symbol%
                // intrinsic reachable. Populate Symbol.prototype first so
                // GetV on that primitive (e.g. Symbol.iterator.toString())
                // can immediately walk the correct prototype.
                this.emitSymbolCtorObject();
            ...
            // [ES2025] RegExp.escape 值读：与构造器物化同槽，保证
            // RegExp.escape === gOPD(RegExp,"escape").value。
            if (propName === "escape" && expr.object.type === "Identifier" &&
                expr.object.name === "RegExp" && !this.regexpNameShadowed()) {
                const escLabel = this.getFunctionLabel("__RE_escape");
                if (escLabel) {
                    this.emitMemoizedBuiltinRef("re_escape", escLabel, "escape");
                    return;
                }
            }
            ...
            // `String.prototype.split` 值读:先物化 Array。转移方法 `__instance.split()`
            // 走 _str_split,槽空则 own constructor 被跳过 → === Array 得 undefined。
            // 仅此稀有反射位内联 Array 物化,不塞进 emitStringCtorObject(编译税)。
            if (propName === "split" && expr.object && expr.object.type === "MemberExpression" &&
                !expr.object.computed && expr.object.property &&
                expr.object.property.type === "Identifier" &&
                expr.object.property.name === "prototype" &&
                expr.object.object && expr.object.object.type === "Identifier" &&
                expr.object.object.name === "String" && !this.stringNameShadowed()) {
                this.emitArrayCtorObject();
            }
            ...
            // %TypedArray%[@@species]:编译期惰装(勿内联进 runtime _ta_intrinsic)。
            if (propName === "Symbol.species" && expr.object && expr.object.type === "Identifier" &&
                expr.object.name === "TypedArray" &&
                !(this.ctx.getLocal && this.ctx.getLocal("TypedArray"))) {
                this.emitEnsureTypedArraySpecies();
            }
            ...
            if (propName === "buffer" || propName === "byteOffset") {
                ...
                if (taObjType === "TypedArray" || taObjType === "unknown" || taObjType === "Object" || propName === "buffer") {
                    if (propName === "buffer") {
                        ...
                        // byteOffset:_ta_byteoffset_prop(装箱;detached/OOB→0)。
                        // 勿直调 `_ta_byteoffset`(内槽在 detach 后仍有效,供 OOB 等内部用)。
                        this.compileExpression(expr.object);
                        this.vm.mov(VReg.A0, VReg.RET);
                        this.vm.call("_ta_byteoffset_prop");
                    }
                    return;
                }
            }
            ...
            if (propName === "length") {
                ...
                if (objType === "Array" || objType === "TypedArray") {
                    // 静态推断 Array 可能被变量重赋冲掉(x=[..]; x={..}; x.length)。
                    // 运行时 tag+type 守卫:非真数组/TA → _js_length_dyn。
                    const id = this.nextLabelId();
                    const dynL = this.ctx.newLabel(`len_dyn_${id}`);
                    const doneL = this.ctx.newLabel(`len_done_${id}`);
                    const slot = this.ctx.allocLocal(`__len_recv_${id}`);
                    this.vm.store(VReg.FP, slot, VReg.RET);
                    if (objType === "TypedArray") {
                        // A TypedArray may have an own data property named
                        // "length" which shadows the inherited intrinsic
                        // accessor.  The generic Get path already implements
                        // side-table-first lookup and the live RAB fallback.
                        this.vm.load(VReg.A0, VReg.FP, slot);
                        this.emitBoxedStringKey("length", VReg.A1);
                        this.vm.call("_object_get");
                        this.vm.mov(VReg.A0, VReg.RET);
                    ...
                    this.vm.load(VReg.A0, VReg.FP, slot);
                    this.vm.shrImm(VReg.V1, VReg.A0, 48);
                    ...
                    if (objType === "TypedArray") {
                        ...
                        this.vm.loadByte(VReg.V1, VReg.RET, 1);
                        this.vm.andImm(VReg.V1, VReg.V1, 32); // ARR_IS_ARGUMENTS
                        this.vm.cmpImm(VReg.V1, 0);
                        this.vm.jne(dynL);
                        this.vm.mov(VReg.A0, VReg.RET);
                    ...
                    this.vm.mov(VReg.A0, VReg.RET);
                    // [I5] 未知/普通 Object 接收者:经 _js_length_dyn。
                    // Object 不可直走 _js_length——其 _js_length_object 把 miss 的 undefined
                    // ToNumber 成 0,破坏 `{}.length === undefined`(shift A2_T1)。
                    // 函数值走 dyn 侧表;String/Number 等静态已知非 Object 仍直走 _js_length。
                    if (objType === "unknown" || objType === "Object") {
                        // [I6] _js_length_dyn 新契约:直接返回装箱 JS 值(canonical number 位,
                        // 或函数值 length 墓碑——删除后读 undefined 不被元数据复活),调用点
                        // 不再做 int→float 转换。
                        if (!isInternalStringSourcePath(this.sourcePath)) {
                            this.vm.call("_js_length_dyn_public");
                        } else {
                            this.vm.call("_js_length_dyn");
                        }
                    } else if (objType === "String" && !isInternalStringSourcePath(this.sourcePath)) {
                        // Public String#length is measured in UTF-16 code units;
                        // `_js_length` intentionally retains the byte-oriented
                        // contract used by the compiler/runtime sources.
                        this.vm.call("_js_length_public");
                        this.vm.scvtf(0, VReg.RET);
                        this.vm.fmovToInt(VReg.RET, 0);
                    } else {
                ...
                if (expr.object.type === "MetaProperty" && propName === "url") {
                    // [layout-determinism] 只嵌入 basename(cwd 无关、node/asm.js 一致)。
                    // 不用 String.replace(/\\/g)：该正则字面量在 members.js 深递归热路径上,
                    // node→gen1 后编译含 import.meta.url 的源码会 SEGV(address=0x1)；
                    // 手写扫反斜杠与 basename 等价且避开 RegExp 快路。
                    const _spb = String(this.sourcePath || ".");
                    let _norm = "";
                    for (let _i = 0; _i < _spb.length; _i++) {
                        const _ch = _spb.charAt(_i);
                        _norm += (_ch === "\\") ? "/" : _ch;
                    }
                    const _slash = _norm.lastIndexOf("/");
                    const _base = (_slash >= 0 ? _norm.slice(_slash + 1) : _norm) || ".";
                ...
                if (arefHelper || strefHelper) {
                    // When the RegExp protocol shim is linked, String method
                    // values must expose the full ECMAScript wrappers rather
                    // than the low-level byte-oriented helpers.  In
                    // particular `String.prototype.matchAll.call(o, r)` and
                    // `.search.call(...)` must perform receiver ToString and
                    // GetMethod/RegExpCreate in the observable order.  Keep
                    // the old helpers for compiler/runtime units that do not
                    // link the shim (the bootstrap compiler itself).
                    let selectedStrefHelper = strefHelper;
                    if (this.ctx.hasFunction && strefHelper) {
                        const fullName = propName === "match" ? "__RE_string_match" :
                            propName === "matchAll" ? "__RE_string_matchAll" :
                            propName === "search" ? "__RE_string_search" : null;
                        if (fullName) {
                            const fullLabel = this.getFunctionLabel && this.getFunctionLabel(fullName);
                            if (fullLabel) selectedStrefHelper = fullLabel;
                        }
                    }
                    const id = this.nextLabelId();
                    ...
                    this.vm.shrImm(VReg.V1, VReg.V1, 48);
                    // 子类化数组(接收者 0x7FFD、[[Prototype]]=数组):fallback 走 IC 前
                    // 须物化 Array.prototype,否则 _object_get_array 槽空 → undefined。
                    if (arefHelper && this.emitArrayProtoObject) {
                        this.emitArrayProtoObject();
                        this.vm.load(VReg.RET, VReg.FP, recvSlot);
                        this.vm.load(VReg.V1, VReg.FP, recvSlot);
                        this.vm.shrImm(VReg.V1, VReg.V1, 48);
                    }
                    if (arefHelper) {
                        ...
                        if (strefHelper) {
                            ...
                            this.emitBuiltinMethodRefClosureMeta(selectedStrefHelper, propName, _sh[1]);
                            this.vm.jmp(endL);
                        } else {
                            // fallback
                            this.vm.load(VReg.RET, VReg.FP, recvSlot);
                        ...
                        // 与 Array.prototype.<m> / emitArrayCtorObject 共用
                        // _builtinref_apm_<m> 槽,使 `[,,,,,].toString ===
                        // Array.prototype.toString`(S11.1.4_A1.2/A1.7)。此前每站点
                        // 新建 24B 闭包,身份漂。调用位 `arr.toString()` 仍走
                        // compileCallExpression 静态派发,不经此。
                        this.emitMemoizedBuiltinMethodRef("apm_" + propName,
                            arefHelper, propName, _ah[1]);
                        this.vm.jmp(endL);
                    } else {
                        // 仅字符串方法
                        const buildStrL = this.ctx.newLabel("aref_str");
                ...
                this.compileExpression(expr.object);
                // A string primitive's named properties are resolved through
                // the lazily materialized String.prototype.  Most common
                // methods have a dedicated Aref value path above, but
                // replace/replaceAll are handled by the full RegExp-aware
                // shim and therefore need the prototype object present when
                // read as a value (`"x".replaceAll.call(...)`).  Limit this
                // to the two shim-backed names so ordinary string property
                // reads (and bootstrap output) stay unchanged.
    ...
    compileMetaProperty(expr) {
        ...
        if (meta === "new" && prop === "target") {
            // NewTarget: prologue snapshots _call_new_target into __new_target.
            // Function identity is the memoized boxed fn; class identity is the
            // same naked classinfo compileIdentifier returns. super() forwards
            // the most-derived value (GetNewTarget).
            const ntOff = this.ctx.getLocal("__new_target");
            if (ntOff != null) {
                this.vm.load(VReg.RET, VReg.FP, ntOff);
            } else {
                this.vm.movImm64(VReg.RET, 0x7ffb000000000000n);
            }
            return;
