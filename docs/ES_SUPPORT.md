...
> 状态基线: **v0.2.7**(2026-07-26;编译器全确定性 `gen1==gen2==gen3`,fixtures 380/380)。
...
> 核对方法: `node tests/run_fixtures.mjs` 全量 + 逐特性最小程序与 `node` 输出逐字节对照 + es-compat 差分计分卡(`tests/es-compat/`,诚实覆盖率 ~42%) + test262 差分(`tests/test262/run.mjs`,当前基线 2,295/6,462 = 35.52%,stride-5 子集;CRASH 177、COMPILE_FAIL 43)。
