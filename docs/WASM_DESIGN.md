...
- **M2(已交付)**:
  ...
  - **fixture harness**:`tests/run_fixtures.mjs` 支持 `ASMJS_FIXTURE_WASM=1`(编 wasm32-wasi、经宿主 shim 运行、产物路径与 native 模式隔离);cli.js 增 `asm.js run --wasm <file.js>`(一步编译+运行)。**R2 增** `expectWasm` 字段:wasm 模式下若 fixture 声明之则整体覆盖 `expect`(native 恒忽略),用于 native 崩溃/不适用但 wasm 语义正确的用例(附加纯增、既有 fixture 零影响)。**2026-07-29 更新**:fixture runner 已统一为权威 `scripts/run-fixtures.mjs`,`tests/run_fixtures.mjs` 降级为薄包装;`ASMJS_FIXTURE_WASM`/`expectWasm` 支路随之暂未被任何 runner 消费(约 4 个 fixture 的 wasm 断言休眠,native 侧 `run:false` 仍 PASS,门禁不破),wasm fixture 回归入口待权威 runner 重新支持,见 `tests/README.md`。
