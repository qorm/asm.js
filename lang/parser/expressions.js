...
export const ExpressionParser = {
    ...
    parseFunctionExpression() {
        ...
        // [test262 yield-as-name] Named function expressions can use yield, await,
        // and other contextual keywords as the binding identifier.
    ...
    parseAsyncExpression() {
        // [test262 for-head-async] `async` is context-sensitive:
        // - `async function` / `async ()` / `async <param> =>` start async expressions.
        // - Otherwise (`async.x`, `async of`, `let async=1`) it is a plain identifier.
        // The next token determines which path we take.
        ...
        // async <param> =>  (e.g. async x =>, async of =>, async yield =>)
        // Any token that can be a binding identifier is a potential async arrow param.
        // We use 2-token lookahead: consume async, check if the following token can be
        // a binding name, then check if it's followed by =>.
        if (next.type === TokenType.IDENT || next.type === TokenType.YIELD ||
            next.type === TokenType.AWAIT || next.type === TokenType.ASYNC ||
            next.type === TokenType.OF || next.type === TokenType.LET ||
            ...
            // Advance past => so parseArrowFunctionBody sees { as curToken
            this.nextToken();
