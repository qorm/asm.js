...
_Generated 2026-09-08T22:06:43.669Z — target linux-arm64 (runner docker: Docker linux/arm64)_
...
- Each assembled test is AOT-compiled by a resident Node compile worker (`new Compiler` + `compileFile` per test; compiler modules loaded once per `--jobs` worker; 8 workers, target `linux-arm64`, 30s timeout) then executed (60s timeout) via `tests/test262/exec-target.mjs` (direct / Rosetta / Docker / Wine according to `--target`; filename-less `t123` binaries never infer the host platform).
...
# 2. run the harness
node tests/test262/run.mjs --stride 5 --jobs 8 --target linux-arm64
```
...
- wall-clock: 1088.1s
- compile-sum (parallel overlap not subtracted): 7433.7s
- run-sum: 1228.1s
- cache warm (hit): 0
- cache cold (miss): 6276 (avg 1184.5ms)

_Run wall-clock: 1088.1s._
