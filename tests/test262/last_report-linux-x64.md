...
_Generated 2026-09-08T23:51:28.758Z — target linux-x64 (runner docker: Docker linux/amd64)_
...
- Each assembled test is AOT-compiled by a resident Node compile worker (`new Compiler` + `compileFile` per test; compiler modules loaded once per `--jobs` worker; 4 workers, target `linux-x64`, 30s timeout) then executed (180s timeout) via `tests/test262/exec-target.mjs` (direct / Rosetta / Docker / Wine according to `--target`; filename-less `t123` binaries never infer the host platform).
...
# 2. run the harness
node tests/test262/run.mjs --stride 5 --jobs 4 --target linux-x64
```
...
- wall-clock: 3083.6s
- compile-sum (parallel overlap not subtracted): 9090.8s
- run-sum: 3219.8s
- cache warm (hit): 0
- cache cold (miss): 6276 (avg 1448.5ms)

_Run wall-clock: 3083.6s._
