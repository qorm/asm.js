...
_Generated 2026-09-08T20:41:57.909Z — target macos-x64 (runner rosetta: Rosetta 2)_
...
- Each assembled test is AOT-compiled by a resident Node compile worker (`new Compiler` + `compileFile` per test; compiler modules loaded once per `--jobs` worker; 4 workers, target `macos-x64`, 30s timeout) then executed (60s timeout) via `tests/test262/exec-target.mjs` (direct / Rosetta / Docker / Wine according to `--target`; filename-less `t123` binaries never infer the host platform).
...
# 2. run the harness
node tests/test262/run.mjs --stride 5 --jobs 4 --target macos-x64
```
...
- wall-clock: 3956.9s
- compile-sum (parallel overlap not subtracted): 8037.1s
- run-sum: 7754.0s
- cache warm (hit): 0
- cache cold (miss): 6276 (avg 1280.6ms)

_Run wall-clock: 3956.9s._
