...
_Generated 2026-09-09T02:21:14.886Z — target windows-x64 (runner wine: wine)_
...
- Each assembled test is AOT-compiled by a resident Node compile worker (`new Compiler` + `compileFile` per test; compiler modules loaded once per `--jobs` worker; 4 workers, target `windows-x64`, 30s timeout) then executed (60s timeout) via `tests/test262/exec-target.mjs` (direct / Rosetta / Docker / Wine according to `--target`; filename-less `t123` binaries never infer the host platform).
...
# 2. run the harness
node tests/test262/run.mjs --stride 5 --jobs 4 --target windows-x64
```
...
- wall-clock: 8899.3s
- compile-sum (parallel overlap not subtracted): 7152.5s
- run-sum: 28011.5s
- cache warm (hit): 0
- cache cold (miss): 6276 (avg 1139.7ms)

_Run wall-clock: 8899.3s._
